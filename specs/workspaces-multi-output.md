...
- **Some workspace operations are still lockstep (known limitation):** while the selector's add ("+") and remove ("×") act on a single output, two paths still add or remove a workspace on *every* output at once and are not yet per-output: (1) window overflow — when a new window needs a free workspace and none exists, the newly created workspace is added to all outputs; (2) fullscreen workspace create/remove — entering/leaving a dedicated fullscreen workspace is applied in lockstep across outputs. The fullscreen-close path also uses the lockstep removal channel (a removal request with no output name means "remove on all outputs", versus a request naming an output which removes only that output's workspace).
