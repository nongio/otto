...
- A selector preview's wallpaper is a mirror of the workspace's wallpaper group, and a mirror only repaints when the node it follows reports a change — a descendant's damage does not reach it while the strip is hidden, which it is until expose opens. So changing the desktop background mid-session must report the change on the wallpaper group itself; otherwise every preview comes up on the next expose still showing the previous wallpaper.
