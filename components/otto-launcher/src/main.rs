...
use otto_launcher::ask::{attached_text, Ask, Note, Status, Step};
...
/// of sessions — to come back from otto-ahp and spring in too.
...
/// How long closing may wait on an ask request still on its way to otto-ahp.
...
struct Launcher {
    ...
    /// Ask mode's connection to otto-ahp, and the request once it is made.
    ask: Option<Ask>,
...
enum Scope {
    ...
    Apps,
    Windows,
    /// What is typed is a request for an agent, handed to otto-ahp.
    Ask,
    /// The agent sessions otto-ahp has, to pick one and carry it on in ask
    /// mode.
    Agents,
}
...
impl Launcher {
    ...
    /// Leave room for the log above the field, or not: a session's field sits
    /// low enough for its log to grow above it, a list of sessions sits where
    /// the launcher's other lists do. The card moves even when its size stays
    /// the same, so the next update places it again.
    fn set_log_room(&mut self, room: bool) {
        if let Some(palette) = self.palette.as_mut() {
            palette.set_log_room(room);
        }
        self.card_size = (0.0, 0.0);
        self.dirty = true;
    }
    ...
    /// round trip to otto-ahp to arrive.
    fn spring(&mut self) {
...
impl App for Launcher {
    fn on_app_ready(&mut self, _ctx: &AppContext) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Room for the log above the field, except over a list of sessions,
        // which has none; see `follow_session` and `back_to_sessions`.
        palette.set_log_room(self.ask.is_some() && !self.picking);
    ...
    fn on_key_event(
        ...
        if control == Some('o') {
            if let Some(terminal) = self.ask.as_ref().and_then(Ask::terminal).cloned() {
                match terminal.open() {
                    Ok(()) => self.close(),
        ...
        // the log, as the page keys always do. Ctrl+C with nothing selected to
        // copy stops the agent's turn; Escape still closes, leaving the agent
        // to it.
        if self.ask_running() {
            let page = self
                .palette
            ...
            if control == Some('c') && !self.input.state.has_selection() {
        ...
        match (event.keysym, control) {
            ...
            (Keysym::Down, _) | (_, Some('n')) if self.can_choose_agent() && !self.choosing_agent => {
    ...
    fn on_pointer_event(&mut self, _ctx: &AppContext, events: &[PointerEvent]) {
        ...
        for event in events {
            ...
            let (screen_x, screen_y) = if card_surface.as_ref() == Some(&event.surface) {
                (event.position.0 as f32 + left, event.position.1 as f32 + top)
            } else {
    ...
    fn on_update(&mut self, _ctx: &AppContext) {
        ...
        if let Some(at) = self.closing_at {
            // A request still on its way to otto-ahp would go with the
