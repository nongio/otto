...
//! Return hands it to otto-ahp, the agent service, and the launcher becomes a
...
use ahp_types::state::{
    AgentInfo, ChatState, ConfirmationOptionKind, Message, MessageAttachment, MessageKind,
    MessageOrigin, MessageResourceAttachment, PendingMessageKind, ResponsePart, SessionStatus,
    SessionSummary, SnapshotState, ToolCallConfirmationReason, ToolCallState, TurnState,
};
...
/// Where otto-ahp listens unless `OTTO_AHP_URL` says otherwise.
...
/// interface: otto-ahp publishes it in the session's `_meta`, under
...
impl Ask {
    /// Connect to otto-ahp in the background. Sessions are created in `$HOME`.
    pub fn open() -> Self {
        let url = std::env::var("OTTO_AHP_URL").unwrap_or_else(|_| DEFAULT_URL.to_string());
    ...
    pub fn connect(url: String, folder: PathBuf) -> Self {
        ...
        std::thread::Builder::new()
            ...
            .expect("cannot start the otto-ahp connection thread");
    ...
    pub fn session_rows(&self, source: usize, query: &str) -> Vec<Item> {
        ...
        self.sessions
            ...
            .map(|(index, session)| Item {
                ...
                subtitle: Some(session_subtitle(session, home.as_deref())),
                icon: None,
                search_terms: Vec::new(),
                origin: Origin { source, index },
            })
    ...
    fn apply(&mut self, update: Update) {
        match update {
            ...
            Update::Unreachable(error) => {
                tracing::warn!(%error, "otto-ahp is unreachable");
            ...
            Update::HandedOff => {
                ...
                tracing::info!("request handed to otto-ahp");
            }
            Update::Failed(error) => {
...
/// What a session in the list is doing, and in which folder.
fn session_subtitle(session: &SessionSummary, home: Option<&Path>) -> String {
    ...
    match folder {
        Some(folder) => format!("{status} · {}", home_relative(&folder, home)),
        None => status,
    }
}
...
async fn serve(
    ...
    let client = match tokio::time::timeout(TIMEOUT, connect(url)).await {
        Ok(Ok(client)) => client,
        Ok(Err(err)) => {
            reporter.send(Update::Unreachable(format!("otto-ahp at {url}: {err}")));
            return;
        }
        Err(_) => {
            reporter.send(Update::Unreachable(format!(
                "otto-ahp at {url} did not answer"
            )));
            return;
        }
    };
    ...
    loop {
        tokio::select! {
            ...
            event = session_events.recv() => match event {
                ...
                Some(_) => {}
                None => {
                    reporter.send(Update::Failed("otto-ahp closed the connection".into()));
                    break;
                }
            },
...
mod tests {
    ...
    /// otto-ahp says how to open a session in a terminal in the session's
    ...
    fn sessions_are_listed_by_title_with_their_folder() {
        ...
        assert_eq!(
            session_subtitle(&ask.sessions[0], Some(home)),
            format!(
                "{} · ~/My Projects",
                otto_kit::t_owned!("launcher-agents-idle")
            )
    ...
    /// Against a live service: `otto-ahp serve --echo`, with `OTTO_AHP_URL`
    /// pointing at it when it is not on the default port.
    #[test]
    #[ignore = "needs a running `otto-ahp serve --echo`"]
    fn follows_a_conversation_to_its_answers() {
        let url = std::env::var("OTTO_AHP_URL").unwrap_or_else(|_| DEFAULT_URL.into());
