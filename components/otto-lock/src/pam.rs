...
/// One of the ten fingers `pam_fprintd` can ask for by name.
...
/// Recognise `pam_fprintd`'s request for a finger, so the panel can say it in
/// the user's language.
///
/// The module's own wording is English unless its catalogues are installed and
/// its process locale matches the panel's, which on a lock screen it need not:
/// "Place your right index finger on Elan Fingerprint Sensor" under an Italian
/// card is the reader talking past the user. What it is asking is a choice from
/// a fixed table, so it can be recognised and said again from ours — the device
/// name goes, because it is hardware's name and the card already has only one
/// line to say this in.
...
/// Whether a PAM error is the reader saying it did not recognise the finger.
///
/// This one message is worth replacing with our own: it is the commonest thing
/// the reader ever says, it arrives in `pam_fprintd`'s English rather than the
/// panel's language, and it carries nothing the user could act on that the
/// catalogue cannot say better. Everything else the module reports — a swipe
/// too short, a finger left on the reader — is guidance, and is passed through
