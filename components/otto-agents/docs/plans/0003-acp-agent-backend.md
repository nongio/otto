...
4. **Renamed on 2026-09-16.** The launcher's Sessions replaced that app
   ([0011](0011-launcher-ask-v2.md)). The service was renamed `otto-agents` and joined
   Otto's Cargo workspace as `components/otto-agents`; `otto-ask` and `otto-agents`
   are aliases for `otto-launcher --ask` and `--agents`.

## Goal

otto-agents runs ACP agents as its child processes. It keeps their sessions alive whether
...
External clients (editors, CLI, other devices) ─────┤ AHP
                                                    ▼
                         otto-agents ── sessions, history, sequencing, auth, automations
                                                    │ ACP over stdio
                                                    ▼
...
- **Sessions outlive clients.** History is persisted under
  `$XDG_STATE_HOME/otto-agents/sessions/`. After a restart, every session is listed as
...
A `crates/otto-agents-acp` crate built on the
...
otto-agents and AHP. The behaviour sections still apply. See
