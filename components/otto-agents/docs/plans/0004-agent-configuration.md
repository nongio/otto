...
**Status:** Shipped. The current reference is `docs/developer/agents.md` (Configuration); this plan is kept as history.
...
A top-level `idle_timeout`, in seconds, stops a session's agent once it has had nothing
to do for that long (default 300; `0` keeps agents running). The next request starts it
again, resuming the agent's session. See 0011, *Idle agents*.

```toml
[ahp]
listen = "127.0.0.1:4800"      # loopback only unless TLS is configured (0005)
socket = true                  # also serve $XDG_RUNTIME_DIR/otto-agents/ahp.sock
...
skills = "claude"              # how the Otto skills reach the agent (0008)
colour = "orange"              # optional; the material the agent's surfaces wear
```

## How it maps to AHP

- **Agent catalogue.** Each agent becomes an entry in `RootState.agents`. `id` becomes
  `provider` and `name` becomes `displayName`; whether the agent can be entered goes
  in `_meta`. Models are asked from the agent at startup.
