...
1. **Typing.** The card is the prompt line alone.
   ...
   - **More than one agent** (changed 2026-09-15): Return sends to the default agent,
     the first in the root state's `agents`, which otto-agents orders by `default_agent`
...
3. **Closing.** Esc or a click outside closes the launcher, as it does today. The
   session carries on, and `otto-agents show --follow` or the launcher's Sessions can pick it up.
...
- **Agent questions go where the user is** (implemented 2026-09-15). Under
  `permissions = "ask"`, otto-agents opens each ACP permission request in the chat as a
  ...
  - **A client is subscribed to the chat,** such as this launcher: otto-agents waits for
...
- **Opening a session.** `otto-launcher --session ID` (a URI, an id or the start of one)
  ...
  typed after that queue on it. Sessions live in the otto-agents process, so a restarted
...
- **Entering a session in a terminal** (added 2026-09-15). Ctrl+O in an open or
  ...
  - Once the agent's id for the session is known, otto-agents publishes the joined command
...
Once a session has had a request, its agent process runs until otto-agents stops.
...
`npm exec` wrapper. otto-agents itself used 11 MiB.
...
- [x] **Tests.** Wrapping, progress from chat state, an unreachable service, and an
      ignored end-to-end test against `otto-agents serve --echo`
...
- [ ] **Try it in Otto.**
- [x] **Questions in the launcher,** routed as above. otto-agents: `host.rs` (questions,
...
- [x] **Idle agents.** Stop a session's agent after `idle_timeout` with nothing running,
      and restart it with resume on the next request. otto-agents: `host.rs`
