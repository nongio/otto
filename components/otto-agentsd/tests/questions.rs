...
use ahp_types::state::{
    ...
    SessionState, SessionStatus, SnapshotState, ToolCallState,
};
...
use otto_agentsd::dialog::{Prompt, Prompter, Reply};
use otto_agentsd::{Server, host, uri};
use serde_json::json;
        ...
        tokio::spawn(async move {
            ...
            while let Some(command) = commands.recv().await {
                ...
                let text = if spec.provider == "questioner" {
                    ...
                    permission(&events, &turn_id).await
                };
...
async fn permission(events: &mpsc::UnboundedSender<SessionEvent>, turn_id: &str) -> String {
    ...
    decision.option_id.unwrap_or_else(|| "none".into())
}
...
async fn connect(dialog: Arc<FakeDialog>) -> Harness {
    ...
    let server = Server::bind_with_prompter("127.0.0.1:0", Arc::new(backend), dialog)
        .await
        .expect("bind");
...
async fn a_watching_client_answers_an_agents_question() {
    let dialog = FakeDialog::answering(false);
    let Harness { client, answers } = connect(Arc::clone(&dialog)).await;
