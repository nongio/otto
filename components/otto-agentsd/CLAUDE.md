...
Otto's agent service: a Rust server for the Agent Host Protocol (AHP).
...
- Use `ahp-types` for wire types. Its version must equal `AHP_PROTOCOL_VERSION` in
  `spec/upstream.env`.
- otto-agentsd is the agents service for Otto: it runs ACP agents and serves them over AHP,
  taking the service role in `../otto/specs/agents.md`. It is a member of Otto's Cargo
  workspace.
  Otto surfaces are AHP clients (`docs/plans/0003`, `0007`, `0011`).
