...
Every task is an otto-agentsd session, so all of them show up in every client: the
...
- **Persistence.** Definitions and run records live in SQLite under
  `$XDG_STATE_HOME/otto-agentsd/`. Each run is recorded before its session is created.
...
```sh
otto-agentsd task "summarise today's commits" --agent claude --folder ~/dev/otto   # start and stream
otto-agentsd task "..." --detach                                                   # print the session URI and exit
otto-agentsd automation add triage --cron "30 9 * * MON-FRI" --tz Europe/Rome \
  --agent claude --folder ~/dev/otto --message "triage new issues"
otto-agentsd automation list | run triage | disable triage | runs triage
```
...
- **Restart recovery:** after otto-agentsd restarts, run records persist and scheduling
  resumes
