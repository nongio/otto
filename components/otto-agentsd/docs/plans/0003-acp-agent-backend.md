...
1. **First draft.** The service (then `otto-ahp`) spawned ACP agents itself.
2. **Brief detour.** The plan was revised to bridge to the `otto-agents` D-Bus service
   specified in `../otto/specs/agents.md`.
3. **Settled on 2026-09-15.** `otto-ahp` became the agent service, taking the role
   Otto's spec gives `otto-agents` and speaking AHP instead of `org.otto.Agents1`. The
   name `otto-agents` briefly went to a GUI app ([0009](0009-otto-agents-app.md)).
4. **Renamed on 2026-09-16.** The launcher's Sessions replaced that app
   ([0011](0011-launcher-ask-v2.md)). The service was renamed `otto-agentsd` and joined
   Otto's Cargo workspace as `components/otto-agentsd`; `otto-ask` and `otto-agents`
   are aliases for `otto-launcher --ask` and `--agents`.

## Goal

otto-agentsd runs ACP agents as its child processes. It keeps their sessions alive whether
or not any client is attached, and presents them as AHP sessions to every client: Otto's
launcher, the islands, editors and the CLI.

```
Otto surfaces (launcher, islands) ─────────────────┐
External clients (editors, CLI, other devices) ─────┤ AHP
                                                    ▼
                         otto-agentsd ── sessions, history, sequencing, auth, automations
                                                    │ ACP over stdio
                                                    ▼
...
- **Sessions outlive clients.** History is persisted under
  `$XDG_STATE_HOME/otto-agentsd/sessions/`. After a restart, every session is listed as
...
A `crates/otto-agentsd-acp` crate built on the
...
otto-agentsd and AHP. The behaviour sections still apply. See
