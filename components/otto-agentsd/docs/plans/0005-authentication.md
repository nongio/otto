...
1. **Client → otto-agentsd.** Only trusted clients may connect. Every connection can start
...
- **Connection token, even on loopback.** Loopback is not a boundary: every local
  ...
  - On first start, write a random token to `$XDG_RUNTIME_DIR/otto-agentsd/token` with mode
    `0600`.
...
- **Origin check.** Reject upgrades that carry a browser `Origin` header not on an
  allowlist.
- **Unix socket.** `$XDG_RUNTIME_DIR/otto-agentsd/ahp.sock` checks peer credentials (same
