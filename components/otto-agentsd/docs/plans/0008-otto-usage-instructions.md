...
read logs. This is Otto agents milestone 5 ("An Otto Guide for Agents"); otto-agentsd owns
the sessions, so otto-agentsd delivers the guide. AHP clients can see exactly what a session
was given.
...
`docs/user/`: `configure-otto` and `extend-otto-files`. otto-agentsd reads it in
`src/skills.rs`, publishes each plugin as a read-only `PluginCustomization` with its
skills as children, and delivers it as a briefing prepended to the first prompt of a
new session, which is the middle option in the table above: no agent we run reads the
directory on its own and ACP has no field for it. The generated guide, the `guide = true`
switch from [0004](0004-agent-configuration.md) (the flag is `skills`, defaulting to on)
and the session toggle are still open.

## otto-agentsd's part

- **Attach per agent.** For agents with `guide = true` ([0004](0004-agent-configuration.md)),
  otto-agentsd attaches the guide to every new session. How it reaches the agent depends on
...
- **otto-agentsd side:** the guide appears in session snapshots, the fake ACP agent receives
