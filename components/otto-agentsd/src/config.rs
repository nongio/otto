...
pub struct AgentConfig {
    ...
    /// How the desktop's skills reach this agent — see [`crate::skills`]. On
    /// by default: an agent on an Otto desktop that does not know how the
    /// desktop works is the thing the skills exist to fix. Turn it off for an
    /// agent whose answers should owe nothing to Otto, or one that already
    /// carries the same guidance.
    #[serde(default)]
    pub skills: SkillDelivery,
}

/// How an agent is given the desktop's skills: `true`, `false` or `"claude"`
/// in `agents.toml`.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub enum SkillDelivery {
    /// Not at all.
    Off,
    /// A briefing ahead of the first prompt, naming each skill and where its
    /// `SKILL.md` is. Works with any agent that can read a file.
    #[default]
    Briefing,
    /// Loaded as Claude Code plugins, through claude-agent-acp's session
    /// options. Claude then knows them as its own skills: `/name` invokes one,
...
mod tests {
    ...
    #[test]
    fn skills_take_a_flag_or_the_claude_plugin_loader() {
        ...
        assert_eq!(skills("").unwrap(), SkillDelivery::Briefing);
        assert_eq!(skills("skills = true").unwrap(), SkillDelivery::Briefing);
    ...
    fn falls_back_to_claude_and_rejects_duplicate_ids() {
        let dir =
            std::env::temp_dir().join(format!("otto-agentsd-config-{}", uuid::Uuid::new_v4()));
