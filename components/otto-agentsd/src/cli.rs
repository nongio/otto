//! Terminal views of the host: `otto-agentsd sessions` and `otto-agentsd show`.
...
async fn connect(url: &str) -> anyhow::Result<Client> {
    let transport = ahp_ws::WebSocketTransport::connect(url)
        .await
        .with_context(|| format!("could not connect to {url}; is `otto-agentsd serve` running?"))?;
    let client = Client::connect(transport, ClientConfig::default()).await?;
    client
        .initialize(
            "otto-agentsd-cli".into(),
