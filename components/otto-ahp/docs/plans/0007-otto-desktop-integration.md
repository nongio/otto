...
- **otto-ahp is the agents service.** It takes the service role from
  `../otto/specs/agents.md`.
- **otto-agents is the GUI** ([0009](0009-otto-agents-app.md)).
- **Every Otto surface is an AHP client** of otto-ahp.
- **D-Bus is used only outbound.** otto-ahp calls Otto's existing renderers, the
...
|---|---|
| M1: The service (D-Bus `org.otto.Agents1`) | otto-ahp: [0002](0002-core-protocol-loop.md), [0003](0003-acp-agent-backend.md), [0004](0004-agent-configuration.md) |
| M2: Ask from the launcher | Launcher "ask" mode as an AHP client (spike below) |
| M3: Session monitor | The otto-agents app ([0009](0009-otto-agents-app.md)) |
| M4: Agents in the islands | Driven by otto-ahp (below) |
...
table in `agents.md` with a pointer to otto-ahp and AHP. The behaviour sections still
apply: states, Enter, questions, notifications, history. Update the milestones to match
the table above.

## How otto-ahp uses Otto's existing services
...
- **Quick permission answers.** Allow / Deny / Enter use Otto's dialog broker
  ...
  - **Implemented (2026-09-15):** under `permissions = "ask"`, each ACP permission
    ...
    subscribed, or once the last watcher leaves, otto-ahp asks through
...
- **Missing services.** otto-ahp never needs `WAYLAND_DISPLAY`. When a D-Bus service is
  missing (for example outside Otto), that integration is skipped and logged once.

## Running otto-ahp in the session

- **Service unit.** A systemd user unit, `otto-ahp.service`, with
...
- **On-demand start.** A socket unit, `otto-ahp.socket`, on
  `$XDG_RUNTIME_DIR/otto-ahp/ahp.sock` starts the service when a client first connects,
  replacing the bus activation in Otto's spec. The service may exit when no session is
  live.
- **Without systemd.** `[[exec_once]] otto-ahp` in Otto's config works, but nothing
  restarts the process if it dies.

## Shared client code for Otto surfaces

The launcher, the otto-agents app and any future surface each need the same
connection logic, which could live in a small `otto-ahp-client` crate on top of `ahp`
...
- **Request.** In `../otto-4`, branch `launcher-ask` from the latest `main`. Start
  `otto-launcher` in an "ask" mode where whatever the user types is sent to otto-ahp.
...
- Should otto-ahp's settings appear in otto-settings through `org.otto.Settings`?
  ([0004](0004-agent-configuration.md))
- Packaging: should otto-ahp ship with Otto's packages (PKGBUILD, nix) or separately?
