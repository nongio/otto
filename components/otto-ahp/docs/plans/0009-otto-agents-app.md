...
sessions otto-ahp is running, and clicking one opens it in its client, either a
terminal or a web page, depending on configuration.

## Behaviour

- **Connecting.** Connect to otto-ahp as an ordinary AHP client, over the Unix socket
...
- **Disconnecting.** When otto-ahp goes away, show a disconnected banner, reconnect
...
- **Toolkit.** An otto-kit app in Otto's `components/otto-agents`, built on the shared
  `otto-ahp-client` crate ([0007](0007-otto-desktop-integration.md)).
- **Protocol only.** The app uses AHP and nothing else, so it also works against a
  remote otto-ahp.
- **Validate the flow first.** A CLI subcommand, `otto-ahp sessions`, can check the
...
- **List:** an end-to-end test against a real otto-ahp with the fake ACP agent asserts
