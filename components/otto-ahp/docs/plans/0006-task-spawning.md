...
Every task is an otto-ahp session, so all of them show up in every client: the
otto-agents app, the islands and editors.
...
- **Persistence.** Definitions and run records live in SQLite under
  `$XDG_STATE_HOME/otto-ahp/`. Each run is recorded before its session is created.
...
```sh
otto-ahp task "summarise today's commits" --agent claude --folder ~/dev/otto   # start and stream
otto-ahp task "..." --detach                                                   # print the session URI and exit
otto-ahp automation add triage --cron "30 9 * * MON-FRI" --tz Europe/Rome \
  --agent claude --folder ~/dev/otto --message "triage new issues"
otto-ahp automation list | run triage | disable triage | runs triage
```
...
- **Restart recovery:** after otto-ahp restarts, run records persist and scheduling
  resumes
