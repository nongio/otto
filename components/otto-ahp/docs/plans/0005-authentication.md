...
1. **Client → otto-ahp.** Only trusted clients may connect. Every connection can start
...
- **Connection token, even on loopback.** Loopback is not a boundary: every local
  ...
  - On first start, write a random token to `$XDG_RUNTIME_DIR/otto-ahp/token` with mode
    `0600`.
...
- **Origin check.** Reject upgrades that carry a browser `Origin` header not on an
  allowlist.
- **Unix socket.** `$XDG_RUNTIME_DIR/otto-ahp/ahp.sock` checks peer credentials (same
...
- When sign-in arrives, should the OAuth flow run in the otto-agents app (as an AHP
