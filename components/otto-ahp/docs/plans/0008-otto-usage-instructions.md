...
read logs. This is Otto agents milestone 5 ("An Otto Guide for Agents"); otto-ahp owns
the sessions, so otto-ahp delivers the guide. AHP clients can see exactly what a session
was given.
...
- **Attach per agent.** For agents with `guide = true` ([0004](0004-agent-configuration.md)),
  otto-ahp attaches the guide to every new session. How it reaches the agent depends on
...
- **otto-ahp side:** the guide appears in session snapshots, the fake ACP agent receives
