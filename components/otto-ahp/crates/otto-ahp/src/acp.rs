...
use crate::config::{self, AgentConfig, PermissionPolicy};
use crate::dialog;
...
impl Backend for AcpBackend {
    fn agents(&self) -> Vec<AgentInfo> {
        self.agents
            .iter()
            .map(|agent| agent_info(&agent.id, &agent.name, &agent.description))
            .collect()
    }
...
/// if it can; otherwise a new session starts.
        ...
        .connect_with(transport, {
            ...
            async move |connection: ConnectionTo<Agent>| {
                let initialized = connection
                    ...
                    .await?;
                let session_id =
                    open_session(&connection, &initialized.agent_capabilities, resume, &cwd)
                        .await?;
