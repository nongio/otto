...
impl Host {
    ...
    fn create_session(
        ...
        let session = Session {
            state: new_session_state(&provider, &uri::from_path(&cwd)),
...
impl HostState {
    ...
    fn restore(&mut self, records: Vec<SessionRecord>) {
        ...
        for (session_uri, chat_uri) in restored {
            ...
            if let Some(turn_id) = interrupted {
                let outcome =
                    TurnOutcome::Failed("otto-ahp stopped before the turn finished".into());
    ...
    fn snapshot(&self, channel: &str) -> Option<Snapshot> {
        ...
        } else if let Some(chat) = self.chats.get(channel) {
            SnapshotState::Chat(Box::new(chat.clone()))
        } else {
            return None;
        };
        Some(Snapshot {
