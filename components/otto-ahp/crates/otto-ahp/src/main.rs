...
use otto_ahp::acp::AcpBackend;
use otto_ahp::agent::{Backend, EchoBackend};
use otto_ahp::dialog::Islands;
use otto_ahp::store::Store;
use otto_ahp::{Server, cli, config};
use tracing_subscriber::EnvFilter;

#[derive(Parser)]
#[command(version, about = "Agent Host Protocol server")]
struct Cli {
    #[command(subcommand)]
...
enum Command {
    ...
    Sessions {
        ...
        #[arg(long, env = "OTTO_AHP_URL", default_value = "ws://127.0.0.1:4800")]
        url: String,
    },
    /// Print a session's transcript.
    Show {
        /// Session id, or the start of one, as listed by `otto-ahp sessions`.
...
struct ServeArgs {
    ...
    #[arg(long, env = "OTTO_AHP_LISTEN", default_value = "127.0.0.1:4800")]
    listen: String,
    /// Agent configuration file. Defaults to Otto's config files.
    #[arg(long, env = "OTTO_AHP_CONFIG")]
    ...
    /// `$XDG_STATE_HOME/otto-ahp/sessions`; with `--echo`, sessions are only
    /// kept when this is given.
    #[arg(long, env = "OTTO_AHP_STATE_DIR")]
...
async fn main() -> anyhow::Result<()> {
    let command = Cli::parse()
        .command
        .unwrap_or_else(|| Command::Serve(ServeArgs::parse_from(["otto-ahp"])));
    match command {
...
async fn serve(args: ServeArgs) -> anyhow::Result<()> {
    ...
    let backend: Arc<dyn Backend> = if args.echo {
        Arc::new(EchoBackend)
    } else {
