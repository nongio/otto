...
use skia_safe::{Canvas, Color, Paint, PathBuilder, Rect, TextBlob};
...
pub fn draw(canvas: &Canvas, x: f32, height: f32, theme: &Theme) -> f32 {
    ...
    if cfg.percentage == PercentagePosition::Inside {
        draw_percentage_inside(canvas, &text, &font, body, theme);
    }

    if battery.state.is_charging() {
        draw_bolt(canvas, body, theme);
    }

    width()
}

/// The percentage over the fill, centred in the body.
fn draw_percentage_inside(
    ...
    let text_width = font.measure_str(text, None).0;
    let (_, metrics) = font.metrics();
    let x = body.left + (body.width() - text_width) / 2.0;
...
/// The charging bolt, over the middle of the glyph.
fn draw_bolt(canvas: &Canvas, body: Rect, theme: &Theme) {
    let h = body.height() * 0.72;
    let w = h * 0.5;
