...
mod live_probe {
    ...
    fn fetches_real_files() {
        ...
        for entry in std::fs::read_dir(&dir).unwrap().flatten().take(8) {
            ...
            let cached = thumbcache::lookup(&path, modified, thumbcache::Size::Large).is_some();
            let job = Job {
                path: path.clone(),
                modified,
                size: thumbcache::Size::Large,
                may_generate: true,
            };
            match fetch(&job) {
                Found::Thumbnail(img) => {
                    ok += 1;
                    eprintln!("OK   {}x{} cached={cached} {}", img.width(), img.height(), path.display());
                }
                Found::Nothing => {
                    fail += 1;
                    eprintln!("MISS cached={cached} {}", path.display());
                }
            }
        }
        eprintln!("=> {ok} found, {fail} missed");
    }
}
