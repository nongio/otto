...
fn with_panel(panel: Rect, display: Rect) -> (Browser, impl Sized) {
    ...
    browser.quickview_panel = Some(panel);
    browser.quickview_placed_offset = Some((0.0, 0.0));
    browser.quickview_display = Some(display);
    (browser, dir)
}
...
/// [`super::Browser::drag_quickview_to`] — but the clamp does.
fn frame(browser: &mut Browser) {
    let offset = offset(browser);
    browser.quickview_panel = Some(panel().with_offset(offset));
    browser.quickview_placed_offset = Some(offset);
}

fn offset(browser: &Browser) -> (f32, f32) {
    browser.quickview.as_ref().expect("a panel").offset
}

#[test]
fn the_title_strip_takes_hold_of_the_panel() {
    let (mut browser, _dir) = with_panel(panel(), display());
    let strip = view::quickview_grip_rect(panel());

    assert!(
        browser.quickview_grip(Point::new(strip.center_x(), strip.center_y()), panel()),
        "a press on the strip starts a drag"
    );
    assert!(browser.quickview_dragging());
}
...
fn the_content_does_not() {
    ...
    assert!(!browser.quickview_grip(Point::new(content.center_x(), content.center_y()), panel()));
    assert!(!browser.quickview_dragging());
}
...
fn the_buttons_are_not_a_handle() {
    let (mut browser, _dir) = with_panel(panel(), display());
    for button in [
        view::quickview_close_rect(panel()),
        view::quickview_expand_rect(panel()),
    ] {
        assert!(
            !browser.quickview_grip(Point::new(button.center_x(), button.center_y()), panel()),
...
fn a_drag_moves_the_panel_by_what_the_pointer_did() {
    ...
    browser.quickview_grip(from, panel());

    browser.drag_quickview_to(Point::new(from.x + 40.0, from.y + 25.0));
...
fn a_drag_over_the_panels_own_surface_does_not_run_away() {
    ...
    let strip = view::quickview_grip_rect(local);
    let grab = Point::new(strip.center_x(), strip.center_y());
    browser.quickview_grip(grab, local);

    for (travelled, expected) in [(10.0, 10.0), (20.0, 20.0), (30.0, 30.0)] {
        browser.drag_quickview_to(Point::new(grab.x + travelled, grab.y));
...
fn a_panel_cannot_be_dragged_off_the_display() {
    ...
    browser.drag_quickview_to(Point::new(from.x + 5000.0, from.y + 5000.0));
    ...
    assert!(
        card.top <= display().bottom - quickview::TITLEBAR_H + 0.5,
...
fn the_panel_is_clamped_back_onto_a_display_that_changed() {
    let (mut browser, _dir) = with_panel(panel(), display());
    browser.quickview.as_mut().expect("a panel").offset = (600.0, 0.0);
    browser.quickview_panel = Some(panel().with_offset((600.0, 0.0)));

    browser.quickview_display = Some(Rect::from_ltrb(0.0, 0.0, 700.0, 600.0));
...
fn the_position_outlives_the_file_but_not_the_panel() {
    let (mut browser, _dir) = with_panel(panel(), display());
    browser.quickview.as_mut().expect("a panel").offset = (40.0, 25.0);

    browser.begin_quickview().expect("the same file again");
...
fn letting_go_ends_the_drag() {
    ...
    assert!(browser.end_quickview_drag(), "it was being dragged");
    assert!(!browser.quickview_dragging());
    assert!(!browser.end_quickview_drag(), "and only the once");
}
...
fn motions_between_two_frames_do_not_stack_up() {
    ...
    for travelled in [10.0, 20.0, 30.0] {
        browser.drag_quickview_to(Point::new(from.x + travelled, from.y));
    }
...
fn clamping_mid_drag_keeps_what_the_drag_asked_for() {
    ...
    let from = Point::new(strip.center_x(), strip.center_y());
    browser.quickview_grip(from, panel());
    browser.drag_quickview_to(Point::new(from.x + 30.0, from.y));
...
fn a_double_click_on_the_title_strip_expands_the_panel() {
    ...
    browser.quickview_grip(at, panel());
    browser.end_quickview_drag();
    assert!(!browser.quickview.as_ref().expect("a panel").expanded);

    assert!(
        browser.quickview_grip(at, panel()),
        "the press is the panel's"
    );
    assert!(
        browser.quickview.as_ref().expect("a panel").expanded,
        "the second press fills the display"
    );
    assert!(!browser.quickview_dragging(), "and starts no drag");
...
fn a_slow_second_press_is_a_drag() {
    ...
    browser.quickview_grip(at, panel());
    browser.end_quickview_drag();
    browser.last_quickview_title_click = Some(std::time::Instant::now() - DOUBLE_CLICK_WINDOW * 2);
