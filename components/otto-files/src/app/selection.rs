...
impl Browser {
    ...
    /// entries' paths while the window holds the keyboard. A folder being
    /// viewed with nothing selected in it is not a selection.
    pub(super) fn focused_selection(&self, focused: bool) -> Vec<String> {
        if !focused {
            return Vec::new();
        }
