---
name: hello-design-component-development
description: Guide for creating and managing HelloDesign components. Use this when asked about HelloDesign component development.
---

# Skill: Creating a HelloDesign Component
...
HelloDesign provides keyboard input constants via `crate::input::keycodes`:

```rust
use crate::input::keycodes;

pub fn handle_keyboard(&mut self, keycode: u32) {
    match keycode {
        keycodes::UP => { /* ... */ }
        keycodes::DOWN => { /* ... */ }
        keycodes::ENTER => { /* ... */ }
...
- `keycodes::UP`, `DOWN`, `LEFT`, `RIGHT` - Arrow keys
- `keycodes::ENTER` - Enter/Return
- `keycodes::ESC` - Escape
- `keycodes::TAB` - Tab
- `keycodes::SPACE` - Space bar
- `keycodes::BACKSPACE` - Backspace
- `keycodes::DELETE` - Delete

Navigation keys:
- `keycodes::HOME`, `END`
- `keycodes::PAGE_UP`, `PAGE_DOWN`

Function keys:
- `keycodes::F1` through `keycodes::F12`

See `components/hello-design/src/input.rs` for complete list.

### Important: Keycodes vs Keysyms

**Current approach** - Linux kernel keycodes (evdev scancodes):
```rust
use crate::input::keycodes;
match keycode {
    keycodes::ENTER => { /* Works but layout-dependent */ }
    _ => {}
}
```

**Better approach** - XKB keysyms (when available):
```rust
use xkbcommon::xkb::keysyms;
match keysym {
    keysyms::KEY_Return => { /* Layout-independent! */ }
    keysyms::KEY_Escape => { /* Cross-platform standard */ }
    _ => {}
}
```
...
When you have XKB context in your event handler:
1. Prefer XKB keysyms from `xkbcommon::xkb::keysyms`
2. They're layout-independent (Ctrl+C works on any layout)
3. Already in dependency tree via `smithay-client-toolkit`

Use `crate::input::keycodes` only when:
- Handling raw `wl_keyboard::key` events before XKB translation
- Matching against raw `u32` keycode values
- XKB context not available

### Example: ContextMenuNext

```rust
use crate::input::keycodes;
use wayland_client::protocol::wl_keyboard;

pub fn handle_keyboard(&mut self, key: u32, key_state: wl_keyboard::KeyState) {
...
- **XKB keysyms**: https://github.com/xkbcommon/libxkbcommon
- **Full list**: See `components/hello-design/src/input.rs`
