...
pub struct ForeignToplevelHandles {
    ...
    /// The output this toplevel is on, for managers that bind later.
    pub output: Option<Output>,
}
```

Methods: `send_title`, `send_app_id`, `send_state`, `send_output_enter`,
`send_output_leave`, `send_done` (ext only, since it batches), `send_closed`.
...
let wlr_handle = self.wlr_foreign_toplevel_state.new_toplevel::<Self>(
    &self.display_handle, &app_id, &title, surface_id.clone(), target_output.as_ref(),
);
self.foreign_toplevels.insert(
    surface_id.clone(),
    ForeignToplevelHandles::new(ext_handle, wlr_handle, target_output),
);
```
...
- **`WlrForeignToplevelManagerState`** is the list of bound manager instances,
  one per connected client. Creating a toplevel broadcasts it to all of them.
- **`WlrToplevelData`** is per-window: app_id, title, current state, and one
...
- **No parent tracking** — transient and child window relationships are not
  exposed.
