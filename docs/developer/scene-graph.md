...
decomposes the screen for the display hardware. A handful of named subtrees
under each output are rendered into buffers of their own, and each buffer is
offered to the KMS plane it was cut out for:
...
port). That is the fastest way to answer "where actually is this layer".
...
| Layer | Holds |
|-------|-------|
| `workspaces_layer` | `background_plane`, `windows_plane`, `promoted_plane`, `expose_layer`. Scrolls horizontally on workspace switch, so all four move in sync for free |
| `overlay_plane` | layer-shell top and overlay, workspace selector, `dock_plane`, OSD, drag-and-drop icon, popups |
| `switcher_plane` | the app switcher |
| `lock_plane` | the lock screen; hidden whenever the session is unlocked |

The dock has a container of its own, `dock_plane`, but it sits inside
`overlay_plane` rather than beside it: a KMS plane costs the full pixel rate
whatever its size, and the chrome fits in one. `promoted_plane` holds the
window promoted to a plane of its own, above `windows_plane` because a
promoted window is always the topmost one.
...
`WindowView` (`src/workspaces/window_view/view.rs`) builds five layers per
window, the base plus four hanging off it in paint order:
...
- **`border_layer`**: the tile outline, hidden unless the window is tiled;
  topmost, because it draws over the client's own edge pixels
...
- **The closure returns its damage rect**, converted from buffer pixels into
  ...
  rect on equal footing. Unlike the return value, those also mark followers as
  needing paint.
...
view.update_state(&new_model);          // hashed; no-op when unchanged
```

`update_state` hashes the model and returns without touching the tree when the
hash matches, so pushing an unchanged model costs nothing. That matters:
`update_decoration` runs on every commit of a decorated window, and
re-rendering unconditionally would repaint the titlebar at the client's frame
rate.

The workspace selector, window selector, background and context menus are all
views. The dock is not: `DockView` drives its layers directly.
...
- **Whole scene**: the default, and the fallback for an output with no
  per-output container.
...
*is* the plane decomposition. `ensure_plane_elements` allocates one element
each for the background, windows, exposé, overlay, promoted window and app
switcher. Most are full-screen; the switcher's is a centred strip, because its
subtree only ever paints there.
...
`change_*` produces a change the engine can animate; `set_*` applies now, with
the caveat that "now" means *when no transition is attached*. Given one,
...
`add_animated_changes`. That distinction is the whole protocol in miniature,
and it is the same one Otto's own UI code uses: clients and the compositor
...
Three runtime levers on the plane path, polled about once a second. They exist
only in `debug-hooks` builds (`--features dev` includes it):
