...
virtual-pointer/keyboard input plumbing is needed.
...
### A native sender (`otto-airplay-tx`), if we ever want one
...
- **Wayland-client component** (recommended): each incoming session creates
  ...
  `zwp_linux_dmabuf_v1`). No compositor changes at all; it composes with
...
evaluate first) or porting UxPlay's session logic (GPLv3: it **cannot** be
...
Otto machine a wireless external monitor for a MacBook. That is worth
verifying against UxPlay first, to confirm third-party receivers get that
mode.
...
3. **Phase 2, a native sender** only if Phase 0 shows demand and we accept
...
- [AirPlay internals: pairing and fp-setup](https://air-display.github.io/airplay-internal/pairing_process.html), [AirPlay 2 internals (Cozzi)](https://emanuelecozzi.net/docs/airplay2)
- [Slave-in-the-Magic-Mirror: the original FairPlay code-execution approach](https://github.com/espes/Slave-in-the-Magic-Mirror)
- [AirplayMirroringGo: a dormant Go sender](https://github.com/openairplay/AirplayMirroringGo)
- [pyatv: pairing and AirPlay-video prior art](https://pyatv.dev)
