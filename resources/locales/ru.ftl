...
settings-frosting = Матовое стекло
settings-frosting-detail = Полупрозрачный размытый материал за доком, панелью и панелями рабочего стола
...
settings-display-language = Язык интерфейса
...
settings-arrangement-hint = Нажмите на экран, чтобы изменить его настройки ниже
...
settings-restart-required = Нужен перезапуск
...
settings-tiling-intro = Мозаичный рабочий стол заполняет экран своими окнами, бок о бок. Это значения по умолчанию; отдельный рабочий стол может задать свои отступы.
settings-tiling-decoration = Оформление мозаичных окон
settings-group-tiling-gaps = Отступы
settings-tiling-inner-gap = Между окнами
settings-tiling-outer-gap = Вокруг окон
settings-tiling-smart-gaps = Без отступов для одного окна
settings-group-tiling-keyboard = Клавиатура
settings-tiling-resize-step = Шаг изменения размера
settings-group-tiling-animation = Анимация
settings-tiling-layout-duration = Смена расположения
...
settings-key-combination-unassigned = Не назначено
# Shown in a shortcut's key field while its record button waits for a
# combination to be pressed.
settings-key-combination-listening = Нажмите клавиши…
...
files-window-title = Файлы
# The Get Info panel's own window.
files-info-window-title = Свойства
...
# Заголовки дней в списке «Недавние», над файлами, сохранёнными в каждый из
# них.
files-recent-today = Сегодня
files-recent-yesterday = Вчера
files-recent-this-week = Ранее на этой неделе
files-recent-this-month = Ранее в этом месяце
files-recent-earlier = Ранее
# Показывается, когда команда, которой нужна папка, — переименование, открытие,
# перемещение в корзину, предпросмотр — применяется к списку, у которого папки
# нет: «Недавние» или результаты поиска.
files-synthetic-no-action = У этого списка нет папки.
files-recent-grid-only = «Недавние» показываются сеткой.
files-recent-not-a-folder = «Недавние» — это список, а не папка.
files-recent-no-location = У «Недавних» нет расположения, к которому можно перейти.

# Полоса фильтра между заголовком окна и списком и то, что она сообщает.
files-search-placeholder = Фильтр: { $folder }
files-search-scope-folder = Эта папка
files-search-scope-everywhere = Везде
files-search-found =
    { $count ->
        [one] { $count } результат
...
files-search-none = Ничего не найдено
files-search-no-columns = У результатов нет столбцов для показа.
# Показывается вместо числа результатов, когда индексирование файлов в системе
# не работает. Поиск и «Недавние» обращаются к нему, поэтому без него ни один
...
files-status-no-items = Нет элементов
files-status-items =
    { $count ->
        [one] { $count } элемент
        [few] { $count } элемента
...
files-status-items-hidden = { $items }, скрытых: { $hidden }
files-status-selected = Выбрано { $count } из { $total }
files-status-opening-preview = Открытие предпросмотра…
files-task-copying = Копирование { $done } из { $total }
files-task-moving = Перемещение { $done } из { $total }
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } из { $total }
files-task-already-running = По одной операции за раз — эта ещё выполняется
...
files-info-owner = Владелец
files-info-links-to = Ссылается на
files-info-permissions = Права доступа
# Column headers over the permission checkboxes — narrower still.
files-perm-read = Чтение
files-perm-write = Запись
files-perm-exec = Выполнение
# Row labels: who each set of permissions applies to.
files-perm-owner = Владелец
files-perm-group = Группа
files-perm-everyone = Все
...
files-picker-save-as-field = Сохранить как:
...
files-save-nowhere = Некуда сохранить
files-save-permission-denied = Нет прав на сохранение здесь

# Confirming an overwrite. $name is a file name, already in quotation marks;
# $count is always two or more.
files-replace-one = «{ $name }» уже существует. Заменить?
...
bar-battery-charging = Батарея { $percent } % — заряжается
bar-battery-plugged = Батарея { $percent } % — подключена, не заряжается
...
schema-frosting-label = Матовое стекло
schema-frosting-description = Полупрозрачный размытый материал за доком, верхней панелью, лаунчером и панелями рабочего стола.
...
# --- tiling ---
schema-tiling-decoration-label = Оформление мозаичных окон
schema-tiling-decoration-description = Сколько оформления сохраняет окно при мозаичном размещении: полоса высотой в одну строку текста, с заголовком и кнопкой закрытия, либо отсутствие полосы, а активное окно отмечено тонкой рамкой.
schema-tiling-inner-gap-label = Отступ между окнами
schema-tiling-inner-gap-description = Логические пиксели между двумя соседними окнами.
schema-tiling-outer-gap-label = Отступ вокруг окон
schema-tiling-outer-gap-description = Логические пиксели между окнами и краем экрана.
schema-tiling-smart-gaps-label = Без отступов для одного окна
schema-tiling-smart-gaps-description = Рабочий стол с единственным окном не оставляет отступов, чтобы одно окно не выглядело без причины вдавленным.
schema-tiling-resize-step-label = Шаг изменения размера
schema-tiling-resize-step-description = Насколько меняется контейнер за один шаг с клавиатуры — долей его ширины или высоты.
schema-tiling-layout-duration-label = Анимация расположения
schema-tiling-layout-duration-description = Секунды, которые занимает смена расположения: окно входит в дерево или покидает его, перемещение, обмен, выравнивание. Ноль переключает сразу.
schema-tiling-layout-bounce-label = Отскок расположения
schema-tiling-layout-bounce-description = Насколько смена расположения проскакивает цель, прежде чем остановиться. Ноль останавливается без отскока.
schema-tiling-mode-duration-label = Анимация мозаики
...
settings-choice-system-language = Системный язык
...
launcher-ask-attached = Вложения: { $files }
# Ask mode, while an existing session is being opened to continue it.
launcher-ask-opening = Открытие сеанса…
launcher-ask-loading = Загрузка беседы…
# The session is going to its terminal window.
launcher-ask-handing-over = Продолжение в терминале…
launcher-ask-handing-over-busy = По завершении текущей работы — продолжение в терминале…
...
launcher-ask-mode-hint = (Shift+Tab — переключение)
...
launcher-ask-step-denied = Отклонено: { $tool }
# Ask mode: under a request in the log, what became of it.
launcher-ask-queued = В очереди
launcher-ask-cancelled = Отменено
launcher-ask-failed = Ошибка: { $error }
# Ask mode, when the agent service is not running to take a request.
launcher-ask-unreachable = Служба агентов не запущена
...
launcher-input-yes = Да
launcher-input-no = Нет
launcher-input-continue = Продолжить
launcher-input-skip = Пропустить вопрос
launcher-input-decline = Не отвечать
launcher-input-open-link = Открыть ссылку
# Sends a request that only asked to open a link.
launcher-input-done = Готово
launcher-input-send = Отправить ответы
# Beside an option the agent suggests.
launcher-input-suggested = Рекомендуется
# What the empty field says while it takes the answer.
launcher-input-type-answer = Ответ…
launcher-input-type-number = Число…
launcher-input-type-other = Или свой ответ…
# In the log, above a question when the agent asked several.
launcher-input-progress = Вопрос { $current } из { $total }
...
launcher-input-declined = Отклонено
launcher-input-dismissed = Закрыто
launcher-input-unanswered = Нет ответа
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = Нужен ответ
launcher-input-error-number = Нужно число
launcher-input-error-integer = Нужно целое число
launcher-input-error-min = Должно быть не меньше { $min }
launcher-input-error-max = Должно быть не больше { $max }
launcher-input-error-short = { $min ->
        [one] Нужно не меньше { $min } символа
        [few] Нужно не меньше { $min } символов
        [many] Нужно не меньше { $min } символов
       *[other] Нужно не меньше { $min } символа
    }
...
launcher-input-error-pick-min = Нужно выбрать не меньше { $min }
launcher-input-error-pick-max = Можно выбрать не больше { $max }
...
agents-permission-title = { $agent } хочет { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = прочитать файл
agents-permission-edit = изменить файл
agents-permission-delete = удалить файл
agents-permission-move = переместить файл
agents-permission-search = выполнить поиск
agents-permission-execute = выполнить команду
agents-permission-fetch = получить данные из интернета
agents-permission-switch-mode = изменить принцип работы
...
emoji-no-results = Эмодзи не найдены
...
emoji-group-recent = Недавние
emoji-group-smileys = Смайлики и эмоции
emoji-group-people = Люди и тело
emoji-group-nature = Животные и природа
emoji-group-food = Еда и напитки
emoji-group-travel = Путешествия и места
emoji-group-activities = Занятия
emoji-group-objects = Предметы
emoji-group-symbols = Символы
...
greeter-status-swipe-finger = Проведите пальцем по сканеру
...
greeter-status-place-named-finger = Коснитесь сканера { $finger }
greeter-status-swipe-named-finger = Проведите по сканеру { $finger }
...
greeter-status-no-match = Отпечаток не распознан
...
# As above, for a swipe reader rather than one you rest a finger on.
lock-status-swipe-finger = Проведите пальцем по сканеру
...
lock-status-place-named-finger = Коснитесь сканера { $finger }
lock-status-swipe-named-finger = Проведите по сканеру { $finger }
...
auth-finger-left-thumb = левым большим пальцем
auth-finger-left-index = левым указательным пальцем
auth-finger-left-middle = левым средним пальцем
auth-finger-left-ring = левым безымянным пальцем
auth-finger-left-little = левым мизинцем
auth-finger-right-thumb = правым большим пальцем
auth-finger-right-index = правым указательным пальцем
auth-finger-right-middle = правым средним пальцем
auth-finger-right-ring = правым безымянным пальцем
auth-finger-right-little = правым мизинцем

# Status line under the fingerprint mark when the reader looked at a finger and
# did not recognise it. The reader asks again straight afterwards, so this is a
# statement, not an instruction. One line.
lock-status-no-match = Отпечаток не распознан
...
peek-pdf-install-rasteriser = Чтобы увидеть страницы, установите один из пакетов: { $packages }
...
peek-empty-folder = Пустая папка
...
peek-error-not-previewable = этот файл нельзя просмотреть
# The file's metadata could not be read.
peek-error-stat-file = не удалось получить сведения о файле: { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = не удалось прочитать файл: { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = в файле невозможно перемещение
...
peek-error-sandbox = не удалось изолировать программу просмотра: { $error }

# Image previewer.
peek-error-read-image = не удалось прочитать изображение: { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = эта сборка не поддерживает такой формат изображения
peek-error-image-no-size = изображение не сообщает свой размер
peek-error-image-decode = изображение не декодировано: { $error }
peek-error-image-readback = не удалось прочитать декодированное изображение

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = не удалось прочитать рисунок: { $error }
peek-error-drawing-parse = не удалось разобрать рисунок
peek-error-drawing-surface = нет поверхности для его отрисовки
peek-error-drawing-readback = не удалось прочитать готовый рисунок

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = это не текст ни в одной из читаемых кодировок

# PDF previewer.
peek-error-read-document = не удалось прочитать документ: { $error }
peek-error-page-readback = не удалось прочитать отрисованную страницу

# Folder listing.
peek-error-read-folder = не удалось прочитать папку
...
peek-error-previewer-missing = не удалось найти программу просмотра: { $error }
peek-error-previewer-start = не удалось запустить программу просмотра: { $error }
peek-error-previewer-no-output = программа просмотра ничего не выдала
peek-error-previewer-unreadable = программа просмотра выдала неразборчивый результат
peek-error-previewer-failed = сбой программы просмотра: { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = просмотр этого файла занял слишком много времени
...
islands-music-elsewhere = Воспроизводится на другом устройстве
...
islands-dialog-page = { $current } из { $total }
...
islands-dialog-multi-hint = Выберите все подходящие
