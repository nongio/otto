...
settings-frosting = Efekt szronu
settings-frosting-detail = Półprzezroczysty, rozmyty materiał za dokiem, paskiem i panelami pulpitu
...
settings-arrangement-hint = Kliknij ekran, aby zmienić jego ustawienia poniżej
...
settings-restart-required = Wymaga restartu
...
settings-agents-intro = Agenci, którym Ask może przekazać prośbę. Uprawnienia decydują, co się dzieje, gdy któryś chce coś zrobić, a jego folder to wszystko, co może odczytać.
settings-agents-default = Domyślny agent
settings-agents-default-detail = Do kogo trafia prośba, gdy nie wybierzesz agenta
# The row holding the Revert and Apply buttons.
settings-agents-changes = Zmiany
settings-agents-revert = Przywróć
settings-agents-apply = Zastosuj
settings-agents-saved = Wszystko jest zapisane
settings-agents-unsaved = Jeszcze nie zapisano. Zastosuj zapisuje agents.toml i ponownie uruchamia usługę agentów.
# $error is the reason, as the system gives it.
settings-agents-failed = Nie udało się zapisać: { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Usługa agentów
settings-agents-service-checking = Sprawdzanie…
settings-agents-service-running = Działa
settings-agents-service-stopped = Zatrzymana. Ask nie dotrze do żadnego agenta, dopóki się nie uruchomi.
settings-agents-service-failed = Zatrzymana po błędzie. Przyczynę podaje journalctl --user -u otto-agents.
settings-agents-service-missing = Nie zainstalowano
...
settings-agents-none-detail = Dodaj ich w { $path }. Przewodnik po Ask i agentach pokazuje, jak to zrobić.
...
settings-agent-name = Nazwa
settings-agents-rename = Zmień nazwę
# The row under the list of agents, holding its Add button.
settings-agents-add = Nowy agent
# What an agent is called until it is renamed.
settings-agents-new-name = Nowy agent
# The program an agent runs on: Claude Code, Codex, OpenCode and so on.
settings-agent-harness = Program
# A harness Settings doesn't recognise, set up by its command alone.
settings-agent-harness-custom = Własny
settings-agent-command = Polecenie
# Which agent file the agent runs as: who it is and how it answers.
settings-agent-instructions = Instrukcje
# No instructions from Otto: the harness runs as itself, Claude Code as
# Claude Code and so on.
settings-agent-instructions-default = Domyślne
settings-agent-instructions-detail = Pliki agentów są w ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = Brak pliku agenta o nazwie { $name }. Umieść go w ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = Hermes potrzebuje do tego profilu: uruchom hermes profile create { $name }
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = Plik instrukcji
settings-agent-permissions = Uprawnienia
settings-agent-permissions-deny = Zawsze odmawiaj
settings-agent-permissions-ask = Pytaj mnie
settings-agent-permissions-allow = Zawsze zezwalaj
...
settings-agent-folder-unset = Nie ustawiono: sesje zaczynają się w { $path }, folderze tymczasowym
settings-agent-folder-detail = Agent może odczytać wszystko, co w nim jest
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Kolor
settings-agent-colour-none = Brak
settings-agent-colour-red = Czerwony
settings-agent-colour-orange = Pomarańczowy
settings-agent-colour-amber = Bursztynowy
settings-agent-colour-yellow = Żółty
settings-agent-colour-lime = Limonkowy
settings-agent-colour-green = Zielony
settings-agent-colour-teal = Morski
settings-agent-colour-cyan = Cyjan
settings-agent-colour-blue = Niebieski
settings-agent-colour-indigo = Indygo
settings-agent-colour-violet = Fioletowy
...
files-info-text = Tekst
files-info-text-reading = Odczytywanie…
...
files-info-text-none = Brak tekstu
files-info-text-unread = Jeszcze nieodczytany
...
files-command-recognise-text = Uruchom rozpoznawanie tekstu
files-recognise-no-pictures = Nie ma tu tekstu do odczytania
...
schema-frosting-label = Efekt szronu
schema-frosting-description = Półprzezroczysty, rozmyty materiał za dokiem, górnym paskiem, launcherem i panelami pulpitu.
...
launcher-ask-handing-over = Kontynuacja w terminalu…
launcher-ask-handing-over-busy = Po zakończeniu bieżącego działania kontynuacja w terminalu…
...
launcher-ask-mode-hint = (Shift+Tab przełącza)
...
launcher-ask-step-denied = Odmówiono: { $tool }
# Ask mode: under a request in the log, what became of it.
launcher-ask-queued = W kolejce
launcher-ask-cancelled = Anulowano
launcher-ask-failed = Niepowodzenie: { $error }
# Ask mode, when the agent service is not running to take a request.
launcher-ask-unreachable = Usługa agentów nie działa
...
launcher-input-yes = Tak
launcher-input-no = Nie
launcher-input-continue = Kontynuuj
launcher-input-skip = Pomiń to pytanie
launcher-input-decline = Nie odpowiadaj
launcher-input-open-link = Otwórz link
# Sends a request that only asked to open a link.
launcher-input-done = Gotowe
launcher-input-send = Wyślij odpowiedzi
# Beside an option the agent suggests.
launcher-input-suggested = Sugerowane
# What the empty field says while it takes the answer.
launcher-input-type-answer = Odpowiedź…
launcher-input-type-number = Liczba…
launcher-input-type-other = Lub własna odpowiedź…
# In the log, above a question when the agent asked several.
launcher-input-progress = Pytanie { $current } z { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Pominięto
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Odmówiono odpowiedzi
launcher-input-dismissed = Zamknięto
launcher-input-unanswered = Bez odpowiedzi
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = To pytanie wymaga odpowiedzi
launcher-input-error-number = To musi być liczba
launcher-input-error-integer = To musi być liczba całkowita
launcher-input-error-min = Musi wynosić co najmniej { $min }
launcher-input-error-max = Musi wynosić najwyżej { $max }
launcher-input-error-short = { $min ->
        [one] Musi mieć co najmniej { $min } znak
        [few] Musi mieć co najmniej { $min } znaki
        [many] Musi mieć co najmniej { $min } znaków
       *[other] Musi mieć co najmniej { $min } znaku
    }
launcher-input-error-long = { $max ->
        [one] Może mieć najwyżej { $max } znak
        [few] Może mieć najwyżej { $max } znaki
        [many] Może mieć najwyżej { $max } znaków
       *[other] Może mieć najwyżej { $max } znaku
    }
launcher-input-error-pick-min = Trzeba wybrać co najmniej { $min }
launcher-input-error-pick-max = Można wybrać najwyżej { $max }
...
agents-permission-title = { $agent } chce { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = odczytać plik
agents-permission-edit = edytować plik
agents-permission-delete = usunąć plik
agents-permission-move = przenieść plik
agents-permission-search = wyszukiwać
agents-permission-execute = wykonać polecenie
agents-permission-fetch = pobrać treść z sieci
agents-permission-switch-mode = zmienić sposób działania
agents-permission-tool = użyć narzędzia
# The dialog's body: the session's folder, as ~/… when it is under home.
agents-permission-in-folder = w { $folder }
...
agents-permission-allow = Zezwól
agents-permission-allow-always = Zawsze zezwalaj
agents-permission-reject = Odrzuć
...
peek-pdf-install-rasteriser = Aby zobaczyć strony, zainstaluj jeden z pakietów: { $packages }
...
peek-empty-folder = Pusty folder
...
peek-error-not-previewable = tego pliku nie można wyświetlić w podglądzie
# The file's metadata could not be read.
peek-error-stat-file = nie można odczytać danych pliku: { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = nie można odczytać pliku: { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = w pliku nie można się przemieszczać
...
peek-error-sandbox = nie można uruchomić podglądu w piaskownicy: { $error }

# Image previewer.
peek-error-read-image = nie można odczytać obrazu: { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = ta wersja nie obsługuje tego formatu obrazu
peek-error-image-no-size = obraz nie podaje swojego rozmiaru
peek-error-image-decode = nie udało się zdekodować obrazu: { $error }
peek-error-image-readback = nie można odczytać zdekodowanego obrazu

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = nie można odczytać rysunku: { $error }
peek-error-drawing-parse = nie udało się przetworzyć rysunku
peek-error-drawing-surface = brak powierzchni do jego wyświetlenia
peek-error-drawing-readback = nie można odczytać narysowanego rysunku

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = ten plik nie jest tekstem w żadnym z odczytywanych kodowań

# PDF previewer.
peek-error-read-document = nie można odczytać dokumentu: { $error }
peek-error-page-readback = nie można odczytać wyrenderowanej strony

# Folder listing.
peek-error-read-folder = nie można odczytać folderu
...
peek-error-previewer-missing = nie można znaleźć programu podglądu: { $error }
peek-error-previewer-start = nie można uruchomić programu podglądu: { $error }
peek-error-previewer-no-output = program podglądu nic nie zwrócił
peek-error-previewer-unreadable = program podglądu zwrócił coś nieczytelnego
peek-error-previewer-failed = program podglądu zakończył się błędem: { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = podgląd tego pliku trwał zbyt długo
...
islands-dialog-page = { $current } z { $total }
...
islands-dialog-multi-hint = Wybierz wszystkie pasujące
