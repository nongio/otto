...
settings-frosting = Effet givré
settings-frosting-detail = Le matériau translucide et flouté derrière le dock, la barre et les panneaux du bureau
...
settings-renderer-no-vulkan-detail = Vulkan n’est pas disponible ici, Otto dessine donc avec OpenGL.
...
settings-key-combination-listening = Appuyez sur les touches…
...
settings-agents-intro = Les agents à qui Ask peut confier une demande. Les autorisations décident de ce qui se passe quand l’un d’eux veut faire quelque chose, et son dossier est tout ce qu’il peut lire.
settings-agents-default = Agent par défaut
settings-agents-default-detail = À qui va une demande quand tu n’en choisis aucun
# The row holding the Revert and Apply buttons.
settings-agents-changes = Modifications
settings-agents-revert = Rétablir
settings-agents-apply = Appliquer
settings-agents-saved = Tout est enregistré
settings-agents-unsaved = Pas encore enregistré. Appliquer écrit agents.toml et redémarre le service des agents.
# $error is the reason, as the system gives it.
settings-agents-failed = Enregistrement impossible : { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Service des agents
settings-agents-service-checking = Vérification…
settings-agents-service-running = Actif
settings-agents-service-stopped = Arrêté. Ask ne peut joindre aucun agent tant qu’il n’a pas démarré.
settings-agents-service-failed = Arrêté après une erreur. journalctl --user -u otto-agents en donne la raison.
settings-agents-service-missing = Non installé
...
settings-agents-none-detail = Ajoute-les dans { $path }. Le guide Ask et agents explique comment.
...
settings-agent-name = Nom
settings-agents-rename = Renommer
# The row under the list of agents, holding its Add button.
settings-agents-add = Nouvel agent
# What an agent is called until it is renamed.
settings-agents-new-name = Nouvel agent
# The program an agent runs on: Claude Code, Codex, OpenCode and so on.
settings-agent-harness = Programme
# A harness Settings doesn't recognise, set up by its command alone.
settings-agent-harness-custom = Personnalisé
settings-agent-command = Commande
...
settings-agent-instructions-default = Par défaut
settings-agent-instructions-detail = Les fichiers d’agent se trouvent dans ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = Aucun fichier d’agent nommé { $name }. Place-le dans ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = Hermes a besoin d’un profil pour cela : lance hermes profile create { $name }
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = Fichier d’instructions
settings-agent-permissions = Autorisations
settings-agent-permissions-deny = Toujours refuser
settings-agent-permissions-ask = Me demander
settings-agent-permissions-allow = Toujours autoriser
settings-agent-model = Modèle
settings-agent-folder = Dossier
# Under an agent's Folder when none is set. $path is Ask's scratch folder,
# such as ~/.local/state/otto/ask.
settings-agent-folder-unset = Non défini : les sessions démarrent dans { $path }, un dossier temporaire
settings-agent-folder-detail = L’agent peut lire tout ce qu’il contient
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Couleur
settings-agent-colour-none = Aucune
settings-agent-colour-red = Rouge
settings-agent-colour-orange = Orange
settings-agent-colour-amber = Ambre
settings-agent-colour-yellow = Jaune
settings-agent-colour-lime = Vert citron
settings-agent-colour-green = Vert
settings-agent-colour-teal = Sarcelle
settings-agent-colour-cyan = Cyan
settings-agent-colour-blue = Bleu
...
files-open-with-window-title = Ouvrir avec
...
files-open-with-mixed = Types de fichiers différents
files-open-with-search = Rechercher des applications
files-open-with-default = Par défaut
files-open-with-other-apps = Autres applications
files-open-with-no-match = Aucune application ne correspond
...
# Les en-têtes de jour de la liste « Récents », au-dessus des fichiers
# enregistrés dans chacun.
files-recent-today = Aujourd’hui
files-recent-yesterday = Hier
files-recent-this-week = Plus tôt cette semaine
files-recent-this-month = Plus tôt ce mois-ci
files-recent-earlier = Plus tôt
# Affiché quand une commande qui exige un dossier — renommer, ouvrir, mettre à
# la corbeille, aperçu — est utilisée sur une liste qui n’en a aucun derrière
# elle : « Récents » ou les résultats d’une recherche.
files-synthetic-no-action = Cette liste ne correspond à aucun dossier.
files-recent-grid-only = « Récents » s’affiche en grille.
files-recent-not-a-folder = « Récents » est une liste, pas un dossier.
files-recent-no-location = « Récents » n’a pas d’emplacement où aller.

# La barre de filtre entre l’en-tête et la liste, et ce qu’elle indique.
files-search-placeholder = Filtrer « { $folder } »
files-search-scope-folder = Ce dossier
files-search-scope-everywhere = Partout
files-search-found =
    { $count ->
        [one] { $count } résultat
...
files-search-none = Aucun résultat
files-search-no-columns = Les résultats n’ont pas de colonnes à afficher.
# Affiché à la place du nombre de résultats quand l’indexation des fichiers du
# système ne tourne pas. La recherche et « Récents » passent toutes deux par
...
attachments-screen-region = Zone de l’écran
files-add-to-stash = Ajouter à la collecte
...
files-task-copying = Copie de { $done } sur { $total }
files-task-moving = Déplacement de { $done } sur { $total }
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } sur { $total }
files-task-already-running = Une opération à la fois — celle-ci continue
...
bar-battery-charging = Batterie { $percent } % — en charge
bar-battery-plugged = Batterie { $percent } % — branchée, pas en charge
...
schema-frosting-label = Effet givré
...
settings-choice-system-language = Langue du système
...
agents-permission-allow = Autoriser
agents-permission-allow-always = Toujours autoriser
agents-permission-reject = Refuser
...
emoji-no-results = Aucun emoji trouvé
...
emoji-group-recent = Récents
emoji-group-smileys = Smileys et émotions
emoji-group-people = Personnes et corps
emoji-group-nature = Animaux et nature
emoji-group-food = Nourriture et boissons
emoji-group-travel = Voyages et lieux
emoji-group-activities = Activités
emoji-group-objects = Objets
emoji-group-symbols = Symboles
emoji-group-flags = Drapeaux
...
peek-pdf-install-rasteriser = Installer l’un de ces paquets : { $packages } — pour voir les pages
...
peek-empty-folder = Dossier vide
...
peek-error-not-previewable = ce n’est pas un fichier dont on puisse avoir un aperçu
# The file's metadata could not be read.
peek-error-stat-file = impossible de lire les informations du fichier : { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = impossible de lire le fichier : { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = le fichier ne permet pas le repositionnement
...
peek-error-sandbox = impossible d’isoler le visualiseur : { $error }

# Image previewer.
peek-error-read-image = impossible de lire l’image : { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = pas une image que cette version sait décoder
peek-error-image-no-size = l’image n’indique aucune taille
peek-error-image-decode = l’image ne s’est pas décodée : { $error }
peek-error-image-readback = impossible de relire l’image décodée

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = impossible de lire le dessin : { $error }
peek-error-drawing-parse = le dessin n’a pas pu être analysé
peek-error-drawing-surface = aucune surface pour l’afficher
peek-error-drawing-readback = impossible de relire le dessin

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = ce fichier n’est du texte dans aucun encodage que l’on lise

# PDF previewer.
peek-error-read-document = impossible de lire le document : { $error }
peek-error-page-readback = impossible de lire la page rendue

# Folder listing.
peek-error-read-folder = impossible de lire le dossier
...
peek-error-previewer-missing = visualiseur introuvable : { $error }
peek-error-previewer-start = impossible de démarrer le visualiseur : { $error }
peek-error-previewer-no-output = le visualiseur n’a rien produit
peek-error-previewer-unreadable = le visualiseur a produit quelque chose d’illisible
peek-error-previewer-failed = le visualiseur s’est interrompu : { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = l’aperçu de ce fichier a pris trop de temps
...
islands-music-elsewhere = Lecture sur un autre appareil
