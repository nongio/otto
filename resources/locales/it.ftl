...
settings-frosting = Effetto smerigliato
settings-frosting-detail = Il materiale traslucido e sfocato dietro il dock, la barra e i pannelli della scrivania
...
settings-display-language = Lingua dell'interfaccia
...
settings-arrangement-hint = Fai clic su uno schermo per modificarne le impostazioni qui sotto
...
settings-restart-required = Riavvio necessario
...
settings-key-combination-unassigned = Non assegnata
# Shown in a shortcut's key field while its record button waits for a
# combination to be pressed.
settings-key-combination-listening = Premi i tasti…
...
settings-agents-intro = Gli agenti a cui Ask può passare una richiesta. I permessi decidono cosa succede quando uno chiede di fare qualcosa, e la sua cartella è tutto ciò che può leggere.
settings-agents-default = Agente predefinito
settings-agents-default-detail = A chi va una richiesta se non ne scegli uno
# The row holding the Revert and Apply buttons.
settings-agents-changes = Modifiche
settings-agents-revert = Ripristina
settings-agents-apply = Applica
settings-agents-saved = È tutto salvato
settings-agents-unsaved = Non ancora salvato. Applica scrive agents.toml e riavvia il servizio degli agenti.
# $error is the reason, as the system gives it.
settings-agents-failed = Salvataggio non riuscito: { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Servizio agenti
settings-agents-service-checking = Verifica…
settings-agents-service-running = Attivo
settings-agents-service-stopped = Fermo. Ask non raggiunge nessun agente finché non parte.
settings-agents-service-failed = Fermato da un errore. journalctl --user -u otto-agents dice perché.
settings-agents-service-missing = Non installato
...
settings-agents-none-detail = Aggiungili in { $path }. La guida Ask e agenti spiega come.
...
settings-agents-rename = Rinomina
# The row under the list of agents, holding its Add button.
settings-agents-add = Nuovo agente
# What an agent is called until it is renamed.
settings-agents-new-name = Nuovo agente
# The program an agent runs on: Claude Code, Codex, OpenCode and so on.
settings-agent-harness = Programma
# A harness Settings doesn't recognise, set up by its command alone.
settings-agent-harness-custom = Personalizzato
settings-agent-command = Comando
# Which agent file the agent runs as: who it is and how it answers.
settings-agent-instructions = Istruzioni
# No instructions from Otto: the harness runs as itself, Claude Code as
# Claude Code and so on.
settings-agent-instructions-default = Predefinite
settings-agent-instructions-detail = I file degli agenti stanno in ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = Nessun file agente di nome { $name }. Mettilo in ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = Hermes ha bisogno di un profilo per questo: esegui hermes profile create { $name }
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = File di istruzioni
settings-agent-permissions = Permessi
settings-agent-permissions-deny = Rifiuta sempre
settings-agent-permissions-ask = Chiedimi
settings-agent-permissions-allow = Consenti sempre
settings-agent-model = Modello
settings-agent-folder = Cartella
# Under an agent's Folder when none is set. $path is Ask's scratch folder,
# such as ~/.local/state/otto/ask.
settings-agent-folder-unset = Non impostata: le sessioni partono in { $path }, una cartella temporanea
settings-agent-folder-detail = L'agente può leggere tutto ciò che contiene
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Colore
settings-agent-colour-none = Nessuno
settings-agent-colour-red = Rosso
settings-agent-colour-orange = Arancione
settings-agent-colour-amber = Ambra
settings-agent-colour-yellow = Giallo
settings-agent-colour-lime = Lime
settings-agent-colour-green = Verde
settings-agent-colour-teal = Verde acqua
settings-agent-colour-cyan = Ciano
settings-agent-colour-blue = Blu
settings-agent-colour-indigo = Indaco
settings-agent-colour-violet = Viola
...
attachments-screen-region = Area dello schermo
files-add-to-stash = Aggiungi alla raccolta
...
files-task-copying = Copia di { $done } di { $total }
files-task-moving = Spostamento di { $done } di { $total }
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } di { $total }
files-task-already-running = Un'operazione alla volta — questa è ancora in corso
...
files-info-text = Testo
files-info-text-reading = Lettura in corso…
files-info-text-words =
    { $count ->
        [one] { $count } parola
       *[other] { $count } parole
    }
files-info-text-none = Nessun testo
files-info-text-unread = Non ancora letto
...
files-command-recognise-text = Esegui il riconoscimento del testo
files-recognise-no-pictures = Qui non c'è testo da leggere
...
bar-battery-percent = Batteria { $percent }%
bar-battery-remaining = Batteria { $percent }% — { $time } rimanenti
bar-battery-charging-time = Batteria { $percent }% — carica completa tra { $time }
bar-battery-full = Batteria { $percent }% — carica completa
bar-battery-charging = Batteria { $percent }% — in carica
bar-battery-plugged = Batteria { $percent }% — collegata, non in carica
bar-cpu-frequency = CPU { $avg } GHz in media, { $max } GHz di picco
bar-cpu-governor = Governor: { $governor }
bar-power-saver = Risparmio energetico
...
schema-accent-color-description = Un nome della tavolozza, che segue gli schemi chiaro e scuro, o un colore #RRGGBB.
...
schema-frosting-label = Effetto smerigliato
...
settings-choice-system-language = Lingua di sistema
...
launcher-search-ask-agent = Chiedi a @{ $agent }…
...
launcher-ask-loading = Caricamento della conversazione…
# The session is going to its terminal window.
launcher-ask-handing-over = Prosegue nel terminale…
launcher-ask-handing-over-busy = Al termine di quanto è in corso, prosegue nel terminale…
...
launcher-ask-mode-hint = (Shift+Tab per cambiare)
...
launcher-input-yes = Sì
launcher-input-no = No
launcher-input-continue = Continua
launcher-input-skip = Salta questa
launcher-input-decline = Non rispondere
launcher-input-open-link = Apri il link
# Sends a request that only asked to open a link.
launcher-input-done = Fine
launcher-input-send = Invia risposte
# Beside an option the agent suggests.
launcher-input-suggested = Suggerita
# What the empty field says while it takes the answer.
launcher-input-type-answer = La tua risposta…
launcher-input-type-number = Un numero…
launcher-input-type-other = Oppure una risposta tua…
# In the log, above a question when the agent asked several.
launcher-input-progress = Domanda { $current } di { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Saltata
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Rifiutata
launcher-input-dismissed = Ignorata
launcher-input-unanswered = Senza risposta
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = Serve una risposta
launcher-input-error-number = Serve un numero
launcher-input-error-integer = Serve un numero intero
launcher-input-error-min = Almeno { $min }
launcher-input-error-max = Al massimo { $max }
launcher-input-error-short = Caratteri: almeno { $min }
launcher-input-error-long = Caratteri: al massimo { $max }
launcher-input-error-pick-min = Scelte: almeno { $min }
launcher-input-error-pick-max = Scelte: al massimo { $max }
...
agents-permission-title = { $agent } vuole { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = leggere un file
agents-permission-edit = modificare un file
agents-permission-delete = eliminare un file
agents-permission-move = spostare un file
agents-permission-search = cercare
agents-permission-execute = eseguire un comando
agents-permission-fetch = recuperare contenuti dal web
agents-permission-switch-mode = cambiare il suo funzionamento
agents-permission-tool = usare uno strumento
# The dialog's body: the session's folder, as ~/… when it is under home.
agents-permission-in-folder = in { $folder }
...
agents-permission-allow = Consenti
agents-permission-allow-always = Consenti sempre
agents-permission-reject = Rifiuta
...
emoji-no-results = Nessuna emoji trovata
...
emoji-group-recent = Recenti
emoji-group-smileys = Faccine ed emozioni
emoji-group-people = Persone e corpo
emoji-group-nature = Animali e natura
emoji-group-food = Cibo e bevande
emoji-group-travel = Viaggi e luoghi
emoji-group-activities = Attività
emoji-group-objects = Oggetti
emoji-group-symbols = Simboli
emoji-group-flags = Bandiere
...
islands-dialog-page = { $current } di { $total }
...
islands-dialog-multi-hint = Più risposte possibili
