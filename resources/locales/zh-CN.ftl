...
settings-frosting = 磨砂效果
settings-frosting-detail = 程序坞、顶栏和桌面面板背后的半透明模糊材质
...
files-task-copying = 正在拷贝第 { $done } 项，共 { $total } 项
files-task-moving = 正在移动第 { $done } 项，共 { $total } 项
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — 第 { $done } 项，共 { $total } 项
files-task-already-running = 一次只能进行一项操作 — 这一项还在进行
...
schema-frosting-label = 磨砂效果
schema-frosting-description = 程序坞、顶栏、启动器和桌面面板背后的半透明模糊材质。
...
agents-permission-allow = 允许
agents-permission-allow-always = 始终允许
agents-permission-reject = 拒绝
...
islands-music-elsewhere = 正在其他设备上播放
