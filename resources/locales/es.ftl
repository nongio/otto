...
settings-frosting = Efecto esmerilado
settings-frosting-detail = El material translúcido y difuminado tras el dock, la barra y los paneles del escritorio
...
settings-arrangement-hint = Haz clic en una pantalla para cambiar sus ajustes abajo
...
settings-restart-required = Requiere reinicio
...
settings-agents-intro = Los agentes a los que Ask puede pasar una petición. Los permisos deciden qué pasa cuando uno pide hacer algo, y su carpeta es todo lo que puede leer.
settings-agents-default = Agente predeterminado
settings-agents-default-detail = A quién va una petición si no eliges ninguno
# The row holding the Revert and Apply buttons.
settings-agents-changes = Cambios
settings-agents-revert = Revertir
settings-agents-apply = Aplicar
settings-agents-saved = Todo está guardado
settings-agents-unsaved = Sin guardar todavía. Aplicar escribe agents.toml y reinicia el servicio de agentes.
# $error is the reason, as the system gives it.
settings-agents-failed = No se pudo guardar: { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Servicio de agentes
settings-agents-service-checking = Comprobando…
settings-agents-service-running = En ejecución
settings-agents-service-stopped = Detenido. Ask no llega a ningún agente hasta que se inicie.
settings-agents-service-failed = Detenido tras un error. journalctl --user -u otto-agents indica el motivo.
settings-agents-service-missing = No instalado
...
settings-agents-none-detail = Añádelos en { $path }. La guía de Ask y agentes explica cómo.
...
settings-agent-name = Nombre
settings-agents-rename = Renombrar
# The row under the list of agents, holding its Add button.
settings-agents-add = Nuevo agente
# What an agent is called until it is renamed.
settings-agents-new-name = Nuevo agente
# The program an agent runs on: Claude Code, Codex, OpenCode and so on.
settings-agent-harness = Programa
# A harness Settings doesn't recognise, set up by its command alone.
settings-agent-harness-custom = Personalizado
settings-agent-command = Comando
# Which agent file the agent runs as: who it is and how it answers.
settings-agent-instructions = Instrucciones
# No instructions from Otto: the harness runs as itself, Claude Code as
# Claude Code and so on.
settings-agent-instructions-default = Predeterminadas
settings-agent-instructions-detail = Los archivos de agente están en ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = No hay ningún archivo de agente llamado { $name }. Ponlo en ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = Hermes necesita un perfil para esto: ejecuta hermes profile create { $name }
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = Archivo de instrucciones
settings-agent-permissions = Permisos
settings-agent-permissions-deny = Rechazar siempre
settings-agent-permissions-ask = Preguntarme
settings-agent-permissions-allow = Permitir siempre
settings-agent-model = Modelo
settings-agent-folder = Carpeta
# Under an agent's Folder when none is set. $path is Ask's scratch folder,
# such as ~/.local/state/otto/ask.
settings-agent-folder-unset = Sin definir: las sesiones empiezan en { $path }, una carpeta temporal
settings-agent-folder-detail = El agente puede leer todo lo que contiene
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Color
settings-agent-colour-none = Ninguno
settings-agent-colour-red = Rojo
settings-agent-colour-orange = Naranja
settings-agent-colour-amber = Ámbar
settings-agent-colour-yellow = Amarillo
settings-agent-colour-lime = Lima
settings-agent-colour-green = Verde
settings-agent-colour-teal = Verde azulado
settings-agent-colour-cyan = Cian
settings-agent-colour-blue = Azul
settings-agent-colour-indigo = Índigo
settings-agent-colour-violet = Violeta
...
files-open-with-window-title = Abrir con
...
files-open-with-mixed = Distintos tipos de archivo
files-open-with-search = Buscar aplicaciones
files-open-with-default = Predeterminada
files-open-with-other-apps = Otras aplicaciones
files-open-with-no-match = Ninguna aplicación coincide
...
# Los encabezados por día de la lista «Recientes», sobre los archivos
# guardados en cada uno.
files-recent-today = Hoy
files-recent-yesterday = Ayer
files-recent-this-week = Esta semana
files-recent-this-month = Este mes
files-recent-earlier = Anteriores
# Se muestra al usar un comando que necesita una carpeta —cambiar nombre, abrir,
# mover a la papelera, previsualizar— en una lista que no tiene ninguna detrás:
# «Recientes» o los resultados de una búsqueda.
files-synthetic-no-action = Esta lista no tiene ninguna carpeta detrás.
files-recent-grid-only = «Recientes» se muestra como cuadrícula.
files-recent-not-a-folder = «Recientes» es una lista, no una carpeta.
files-recent-no-location = «Recientes» no tiene una ubicación a la que ir.

# La barra de filtro entre la cabecera y la lista, y lo que informa.
files-search-placeholder = Filtrar «{ $folder }»
files-search-scope-folder = Esta carpeta
files-search-scope-everywhere = En todas partes
files-search-found =
    { $count ->
        [one] { $count } resultado
...
files-search-none = No se encontró nada
files-search-no-columns = Los resultados no tienen columnas que mostrar.
# Se muestra en lugar del número de resultados cuando la indexación de archivos
# del sistema no está en marcha. La búsqueda y «Recientes» pasan por ella, así
...
attachments-screen-region = Región de la pantalla
files-add-to-stash = Añadir a la recopilación
...
files-task-copying = Copiando { $done } de { $total }
files-task-moving = Moviendo { $done } de { $total }
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } de { $total }
files-task-already-running = Una operación cada vez — esta todavía sigue
...
files-info-text = Texto
files-info-text-reading = Leyendo…
files-info-text-words =
    { $count ->
        [one] { $count } palabra
       *[other] { $count } palabras
    }
files-info-text-none = Sin texto
files-info-text-unread = Aún sin leer
...
files-command-recognise-text = Ejecutar reconocimiento de texto
files-recognise-no-pictures = Aquí no hay texto que leer
...
bar-battery-percent = Batería { $percent } %
bar-battery-remaining = Batería { $percent } % — quedan { $time }
bar-battery-charging-time = Batería { $percent } % — { $time } para la carga completa
bar-battery-full = Batería { $percent } % — carga completa
bar-battery-charging = Batería { $percent } % — cargando
bar-battery-plugged = Batería { $percent } % — conectada, sin cargar
bar-cpu-frequency = CPU { $avg } GHz de media, { $max } GHz de pico
bar-cpu-governor = Gobernador: { $governor }
bar-power-saver = Ahorro de energía
...
schema-frosting-label = Efecto esmerilado
schema-frosting-description = El material translúcido y difuminado tras el dock, la barra superior, el lanzador y los paneles del escritorio.
...
launcher-ask-opening = Abriendo la sesión…
launcher-ask-loading = Cargando la conversación…
# The session is going to its terminal window.
launcher-ask-handing-over = Continuando en el terminal…
launcher-ask-handing-over-busy = Terminando lo que está en curso y continuando en el terminal…
...
launcher-ask-mode-hint = (Shift+Tab para cambiar)
...
launcher-ask-step-denied = Denegado: { $tool }
# Ask mode: under a request in the log, what became of it.
launcher-ask-queued = En cola
launcher-ask-cancelled = Cancelada
launcher-ask-failed = Error: { $error }
# Ask mode, when the agent service is not running to take a request.
launcher-ask-unreachable = El servicio de agentes no está en ejecución
...
launcher-input-yes = Sí
launcher-input-no = No
launcher-input-continue = Continuar
launcher-input-skip = Omitir esta
launcher-input-decline = No responder
launcher-input-open-link = Abrir el enlace
# Sends a request that only asked to open a link.
launcher-input-done = Listo
launcher-input-send = Enviar respuestas
# Beside an option the agent suggests.
launcher-input-suggested = Sugerida
# What the empty field says while it takes the answer.
launcher-input-type-answer = Tu respuesta…
launcher-input-type-number = Un número…
launcher-input-type-other = O una respuesta propia…
# In the log, above a question when the agent asked several.
launcher-input-progress = Pregunta { $current } de { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Omitida
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Rechazada
launcher-input-dismissed = Descartada
launcher-input-unanswered = Sin responder
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = Esta necesita una respuesta
launcher-input-error-number = Tiene que ser un número
launcher-input-error-integer = Tiene que ser un número entero
launcher-input-error-min = Como mínimo { $min }
launcher-input-error-max = Como máximo { $max }
launcher-input-error-short = Caracteres: como mínimo { $min }
launcher-input-error-long = Caracteres: como máximo { $max }
launcher-input-error-pick-min = Opciones: como mínimo { $min }
launcher-input-error-pick-max = Opciones: como máximo { $max }
...
agents-permission-title = { $agent } quiere { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = leer un archivo
agents-permission-edit = editar un archivo
agents-permission-delete = eliminar un archivo
agents-permission-move = mover un archivo
agents-permission-search = buscar
agents-permission-execute = ejecutar un comando
agents-permission-fetch = obtener contenido de la web
agents-permission-switch-mode = cambiar su funcionamiento
agents-permission-tool = usar una herramienta
# The dialog's body: the session's folder, as ~/… when it is under home.
agents-permission-in-folder = en { $folder }
...
agents-permission-allow = Permitir
agents-permission-allow-always = Permitir siempre
agents-permission-reject = Rechazar
...
emoji-search = Buscar emojis…
# Shown in the grid when nothing matches what was typed.
emoji-no-results = No se han encontrado emojis
...
emoji-group-recent = Recientes
emoji-group-smileys = Emoticonos y emociones
emoji-group-people = Personas y cuerpo
emoji-group-nature = Animales y naturaleza
emoji-group-food = Comida y bebida
emoji-group-travel = Viajes y lugares
...
peek-pdf-install-rasteriser = Instalar uno de estos: { $packages } — para ver las páginas
...
peek-empty-folder = Carpeta vacía
...
peek-error-not-previewable = no es un archivo del que se pueda ver una previsualización
# The file's metadata could not be read.
peek-error-stat-file = no se puede leer la información del archivo: { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = no se puede leer el archivo: { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = el archivo no permite reposicionarse
...
peek-error-sandbox = no se puede aislar el visualizador: { $error }

# Image previewer.
peek-error-read-image = no se puede leer la imagen: { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = no es una imagen que esta versión sepa descodificar
peek-error-image-no-size = la imagen no indica ningún tamaño
peek-error-image-decode = la imagen no se ha descodificado: { $error }
peek-error-image-readback = no se puede releer la imagen descodificada

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = no se puede leer el dibujo: { $error }
peek-error-drawing-parse = no se ha podido analizar el dibujo
peek-error-drawing-surface = no hay ninguna superficie donde representarlo
peek-error-drawing-readback = no se puede releer el dibujo

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = este archivo no es texto en ninguna codificación que se lea

# PDF previewer.
peek-error-read-document = no se puede leer el documento: { $error }
peek-error-page-readback = no se puede leer la página generada

# Folder listing.
peek-error-read-folder = no se puede leer la carpeta
...
peek-error-previewer-missing = no se encuentra el visualizador: { $error }
peek-error-previewer-start = no se puede iniciar el visualizador: { $error }
peek-error-previewer-no-output = el visualizador no ha producido nada
peek-error-previewer-unreadable = el visualizador ha producido algo ilegible
peek-error-previewer-failed = el visualizador se ha interrumpido: { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = este archivo ha tardado demasiado en previsualizarse
...
islands-dialog-page = { $current } de { $total }
...
islands-dialog-multi-hint = Selecciona las que correspondan
