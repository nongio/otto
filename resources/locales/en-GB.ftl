...
settings-frosting = Frosting
settings-frosting-detail = The translucent, blurred material behind the dock, the bar and the desktop's panels
...
settings-renderer-no-vulkan-detail = Vulkan isn't in this build, so Otto draws with OpenGL.
...
# Shown when a command that needs a folder — rename, open, trash, Peek —
...
# long one loop of an animation lasts, as minutes and seconds.
...
schema-frosting-label = Frosting
...
launcher-search-ask-agent = Ask @{ $agent }…
...
launcher-ask-handing-over = Carrying on in the terminal…
launcher-ask-handing-over-busy = Finishing this turn, then carrying on in the terminal…
...
launcher-ask-mode-hint = (Shift+Tab to switch)
...
launcher-input-yes = Yes
launcher-input-no = No
launcher-input-continue = Continue
launcher-input-skip = Skip this one
launcher-input-decline = Don’t answer
launcher-input-open-link = Open the link
# Sends a request that only asked to open a link.
launcher-input-done = Done
launcher-input-send = Send answers
# Beside an option the agent suggests.
launcher-input-suggested = Suggested
# What the empty field says while it takes the answer.
launcher-input-type-answer = Your answer…
launcher-input-type-number = A number…
launcher-input-type-other = Or an answer of your own…
# In the log, above a question when the agent asked several.
launcher-input-progress = Question { $current } of { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Skipped
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Declined
launcher-input-dismissed = Dismissed
launcher-input-unanswered = Not answered
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = This one needs an answer
launcher-input-error-number = That needs to be a number
launcher-input-error-integer = That needs to be a whole number
launcher-input-error-min = It needs to be at least { $min }
launcher-input-error-max = It needs to be at most { $max }
launcher-input-error-short = It needs at least { $min } characters
launcher-input-error-long = It can be at most { $max } characters
launcher-input-error-pick-min = It needs at least { $min } picked
launcher-input-error-pick-max = It takes at most { $max }
...
agents-permission-title = { $agent } wants to { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = read a file
agents-permission-edit = edit a file
agents-permission-delete = delete a file
agents-permission-move = move a file
agents-permission-search = search
agents-permission-execute = run a command
agents-permission-fetch = fetch from the web
agents-permission-switch-mode = change how it works
agents-permission-tool = use a tool
...
agents-permission-allow = Allow
agents-permission-allow-always = Always allow
agents-permission-reject = Reject
...
peek-pdf-install-rasteriser = Install one of: { $packages } — to see the pages
...
peek-empty-folder = Empty folder
...
peek-error-not-previewable = this is not a file that can be previewed
# The file's metadata could not be read.
peek-error-stat-file = cannot stat the file: { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = cannot read the file: { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = the file is not seekable
...
peek-error-sandbox = could not sandbox the previewer: { $error }

# Image previewer.
peek-error-read-image = cannot read the image: { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = not an image this build can decode
peek-error-image-no-size = the image reports no size
peek-error-image-decode = the image did not decode: { $error }
peek-error-image-readback = the decoded image could not be read back

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = cannot read the drawing: { $error }
peek-error-drawing-parse = the drawing could not be parsed
peek-error-drawing-surface = no surface to render it on
peek-error-drawing-readback = the drawing could not be read back

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = this file is not text in any encoding Otto reads

# PDF previewer.
peek-error-read-document = cannot read the document: { $error }
peek-error-page-readback = the rendered page could not be read

# Folder listing.
peek-error-read-folder = cannot read the folder
...
peek-error-previewer-missing = cannot find the previewer: { $error }
peek-error-previewer-start = cannot start the previewer: { $error }
peek-error-previewer-no-output = the previewer produced no output
peek-error-previewer-unreadable = the previewer produced something unreadable
peek-error-previewer-failed = the previewer failed: { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = this file took too long to preview
