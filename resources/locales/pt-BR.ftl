...
settings-frosting = Efeito fosco
settings-frosting-detail = O material translúcido e desfocado atrás do dock, da barra e dos painéis da área de trabalho
...
settings-arrangement-hint = Clique em uma tela para alterar suas configurações abaixo
...
settings-restart-required = Requer reinício
...
settings-key-combination-unassigned = Não atribuído
# Shown in a shortcut's key field while its record button waits for a
# combination to be pressed.
settings-key-combination-listening = Pressione as teclas…
...
settings-agents-intro = Os agentes a quem o Ask pode passar um pedido. As permissões decidem o que acontece quando um deles pede para fazer algo, e a pasta dele é tudo o que ele pode ler.
settings-agents-default = Agente padrão
settings-agents-default-detail = Quem recebe um pedido quando você não escolhe nenhum
# The row holding the Revert and Apply buttons.
settings-agents-changes = Alterações
settings-agents-revert = Reverter
settings-agents-apply = Aplicar
settings-agents-saved = Tudo está salvo
settings-agents-unsaved = Ainda não salvo. Aplicar grava o agents.toml e reinicia o serviço de agentes.
# $error is the reason, as the system gives it.
settings-agents-failed = Não foi possível salvar: { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Serviço de agentes
settings-agents-service-checking = Verificando…
settings-agents-service-running = Em execução
settings-agents-service-stopped = Parado. O Ask não alcança nenhum agente até que ele inicie.
settings-agents-service-failed = Parado após um erro. journalctl --user -u otto-agents mostra o motivo.
settings-agents-service-missing = Não instalado
...
settings-agents-none-detail = Adicione-os em { $path }. O guia do Ask e dos agentes mostra como.
...
settings-agent-name = Nome
settings-agents-rename = Renomear
# The row under the list of agents, holding its Add button.
settings-agents-add = Novo agente
# What an agent is called until it is renamed.
settings-agents-new-name = Novo agente
...
settings-agent-instructions = Instruções
# No instructions from Otto: the harness runs as itself, Claude Code as
# Claude Code and so on.
settings-agent-instructions-default = Padrão
settings-agent-instructions-detail = Os arquivos de agente ficam em ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = Nenhum arquivo de agente chamado { $name }. Coloque-o em ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = O Hermes precisa de um perfil para isso: execute hermes profile create { $name }
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = Arquivo de instruções
settings-agent-permissions = Permissões
settings-agent-permissions-deny = Recusar sempre
settings-agent-permissions-ask = Me perguntar
settings-agent-permissions-allow = Permitir sempre
settings-agent-model = Modelo
settings-agent-folder = Pasta
# Under an agent's Folder when none is set. $path is Ask's scratch folder,
# such as ~/.local/state/otto/ask.
settings-agent-folder-unset = Não definida: as sessões começam em { $path }, uma pasta temporária
settings-agent-folder-detail = O agente pode ler tudo o que há nela
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Cor
settings-agent-colour-none = Nenhuma
settings-agent-colour-red = Vermelho
settings-agent-colour-orange = Laranja
settings-agent-colour-amber = Âmbar
settings-agent-colour-yellow = Amarelo
settings-agent-colour-lime = Lima
settings-agent-colour-green = Verde
settings-agent-colour-teal = Verde-azulado
settings-agent-colour-cyan = Ciano
...
files-open-with-window-title = Abrir com
...
files-open-with-mixed = Tipos de arquivo diferentes
files-open-with-search = Pesquisar aplicativos
files-open-with-default = Padrão
files-open-with-other-apps = Outros aplicativos
files-open-with-no-match = Nenhum aplicativo corresponde
...
files-task-moving = Movendo { $done } de { $total }
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } de { $total }
files-task-already-running = Uma operação por vez — esta ainda está em andamento
...
files-info-text-reading = Lendo…
files-info-text-words =
    { $count ->
        [one] { $count } palavra
       *[other] { $count } palavras
    }
files-info-text-none = Sem texto
files-info-text-unread = Ainda não lido
...
files-command-recognise-text = Executar reconhecimento de texto
files-recognise-no-pictures = Não há texto para ler aqui
...
bar-battery-remaining = Bateria { $percent }% — { $time } restantes
bar-battery-charging-time = Bateria { $percent }% — carga completa em { $time }
bar-battery-full = Bateria { $percent }% — totalmente carregada
bar-battery-charging = Bateria { $percent }% — carregando
bar-battery-plugged = Bateria { $percent }% — conectada, sem carregar
bar-cpu-frequency = CPU { $avg } GHz em média, pico de { $max } GHz
bar-cpu-governor = Governador: { $governor }
bar-power-saver = Economia de energia
...
schema-frosting-label = Efeito fosco
schema-frosting-description = O material translúcido e desfocado atrás do dock, da barra superior, do lançador e dos painéis da área de trabalho.
...
launcher-ask-attached = Anexos: { $files }
# Ask mode, while an existing session is being opened to continue it.
launcher-ask-opening = Abrindo a sessão…
launcher-ask-loading = Carregando a conversa…
# The session is going to its terminal window.
launcher-ask-handing-over = Continuando no terminal…
launcher-ask-handing-over-busy = Terminando o que está em andamento e continuando no terminal…
...
launcher-ask-mode-hint = (Shift+Tab para alternar)
...
launcher-ask-step-denied = Negado: { $tool }
# Ask mode: under a request in the log, what became of it.
launcher-ask-queued = Na fila
launcher-ask-cancelled = Cancelada
launcher-ask-failed = Falhou: { $error }
# Ask mode, when the agent service is not running to take a request.
launcher-ask-unreachable = O serviço de agentes não está em execução
...
launcher-input-yes = Sim
launcher-input-no = Não
launcher-input-continue = Continuar
launcher-input-skip = Pular esta
launcher-input-decline = Não responder
launcher-input-open-link = Abrir o link
# Sends a request that only asked to open a link.
launcher-input-done = Concluído
launcher-input-send = Enviar respostas
# Beside an option the agent suggests.
launcher-input-suggested = Sugerida
# What the empty field says while it takes the answer.
launcher-input-type-answer = Sua resposta…
launcher-input-type-number = Um número…
launcher-input-type-other = Ou uma resposta sua…
# In the log, above a question when the agent asked several.
launcher-input-progress = Pergunta { $current } de { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Pulada
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Recusada
launcher-input-dismissed = Descartada
launcher-input-unanswered = Sem resposta
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = Esta precisa de uma resposta
launcher-input-error-number = Precisa ser um número
launcher-input-error-integer = Precisa ser um número inteiro
launcher-input-error-min = No mínimo { $min }
launcher-input-error-max = No máximo { $max }
launcher-input-error-short = Caracteres: no mínimo { $min }
launcher-input-error-long = Caracteres: no máximo { $max }
launcher-input-error-pick-min = Opções: no mínimo { $min }
launcher-input-error-pick-max = Opções: no máximo { $max }
...
agents-permission-title = { $agent } quer { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = ler um arquivo
agents-permission-edit = editar um arquivo
agents-permission-delete = excluir um arquivo
agents-permission-move = mover um arquivo
agents-permission-search = buscar
agents-permission-execute = executar um comando
agents-permission-fetch = obter conteúdo da web
agents-permission-switch-mode = mudar seu funcionamento
agents-permission-tool = usar uma ferramenta
# The dialog's body: the session's folder, as ~/… when it is under home.
agents-permission-in-folder = em { $folder }
...
agents-permission-allow-always = Permitir sempre
agents-permission-reject = Recusar
...
emoji-search = Pesquisar emojis…
# Shown in the grid when nothing matches what was typed.
emoji-no-results = Nenhum emoji encontrado
...
emoji-group-recent = Recentes
emoji-group-smileys = Smileys e emoções
emoji-group-people = Pessoas e corpo
emoji-group-nature = Animais e natureza
emoji-group-food = Comida e bebida
emoji-group-travel = Viagens e lugares
emoji-group-activities = Atividades
emoji-group-objects = Objetos
emoji-group-symbols = Símbolos
emoji-group-flags = Bandeiras
...
peek-pdf-install-rasteriser = Para ver as páginas, instale um destes: { $packages }
...
peek-error-not-previewable = este não é um arquivo que possa ser visualizado
# The file's metadata could not be read.
peek-error-stat-file = não é possível obter os dados do arquivo: { $error }
# The file's bytes could not be read. Also used by the text previewer.
peek-error-read-file = não é possível ler o arquivo: { $error }
# The file cannot be rewound, so it cannot be identified and then read.
peek-error-not-seekable = o arquivo não permite reposicionamento
...
peek-error-sandbox = não foi possível isolar o visualizador: { $error }

# Image previewer.
peek-error-read-image = não é possível ler a imagem: { $error }
# The bytes are an image format this build has no decoder for.
peek-error-image-unsupported = esta compilação não decodifica este formato de imagem
peek-error-image-no-size = a imagem não informa seu tamanho
peek-error-image-decode = a imagem não foi decodificada: { $error }
peek-error-image-readback = não foi possível reler a imagem decodificada

# SVG previewer. "the drawing" means the SVG, as distinct from a photograph.
peek-error-read-drawing = não é possível ler o desenho: { $error }
peek-error-drawing-parse = não foi possível interpretar o desenho
peek-error-drawing-surface = nenhuma superfície para exibi-lo
peek-error-drawing-readback = não foi possível reler o desenho

# Text previewer: the bytes are not text in UTF-8 or in Latin-1.
peek-error-not-text = este arquivo não é texto em nenhuma codificação que o Otto lê

# PDF previewer.
peek-error-read-document = não é possível ler o documento: { $error }
peek-error-page-readback = não foi possível ler a página renderizada

# Folder listing.
peek-error-read-folder = não é possível ler a pasta
...
peek-error-previewer-missing = não é possível encontrar o visualizador: { $error }
peek-error-previewer-start = não é possível iniciar o visualizador: { $error }
peek-error-previewer-no-output = o visualizador não produziu nada
peek-error-previewer-unreadable = o visualizador produziu algo ilegível
peek-error-previewer-failed = o visualizador falhou: { $error }
# The worker was still going after the deadline and was killed.
peek-error-timeout = este arquivo demorou demais para ser visualizado
...
islands-music-elsewhere = Tocando em outro dispositivo
...
islands-dialog-multi-hint = Escolha todas as que se aplicam
