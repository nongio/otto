...
settings-frosting = Milchglas
settings-frosting-detail = Das durchscheinende, unscharfe Material hinter Dock, Leiste und den Panels des Desktops
...
settings-key-combination-listening = Tasten drücken …
...
settings-agents-intro = Die Agenten, an die Ask eine Anfrage weitergeben kann. Die Berechtigungen legen fest, was passiert, wenn einer etwas tun will, und sein Ordner ist alles, was er lesen kann.
settings-agents-default = Standardagent
settings-agents-default-detail = Wer eine Anfrage bekommt, wenn du keinen wählst
# The row holding the Revert and Apply buttons.
settings-agents-changes = Änderungen
settings-agents-revert = Zurücksetzen
settings-agents-apply = Anwenden
settings-agents-saved = Alles ist gespeichert
settings-agents-unsaved = Noch nicht gespeichert. „Anwenden“ schreibt agents.toml und startet den Agentendienst neu.
# $error is the reason, as the system gives it.
settings-agents-failed = Speichern nicht möglich: { $error }
# The otto-agents service, which runs the agents; the row shows whether it is
# up, with Start or Restart.
settings-agents-service = Agentendienst
settings-agents-service-checking = Wird geprüft…
settings-agents-service-running = Läuft
settings-agents-service-stopped = Gestoppt. Ask erreicht keinen Agenten, bis er startet.
settings-agents-service-failed = Nach einem Fehler gestoppt. journalctl --user -u otto-agents nennt den Grund.
settings-agents-service-missing = Nicht installiert
...
settings-agents-none-detail = Füge sie in { $path } hinzu. Die Anleitung zu Ask und Agenten zeigt, wie.
...
settings-agents-rename = Umbenennen
# The row under the list of agents, holding its Add button.
settings-agents-add = Neuer Agent
# What an agent is called until it is renamed.
settings-agents-new-name = Neuer Agent
# The program an agent runs on: Claude Code, Codex, OpenCode and so on.
settings-agent-harness = Programm
# A harness Settings doesn't recognise, set up by its command alone.
settings-agent-harness-custom = Eigenes
settings-agent-command = Befehl
# Which agent file the agent runs as: who it is and how it answers.
settings-agent-instructions = Anweisungen
# No instructions from Otto: the harness runs as itself, Claude Code as
# Claude Code and so on.
settings-agent-instructions-default = Standard
settings-agent-instructions-detail = Agentendateien liegen in ~/.local/share/otto/plugins/<plugin>/agents/
# $name is the agent file the agent names, which wasn't found.
settings-agent-instructions-missing = Keine Agentendatei namens { $name }. Leg sie in ~/.local/share/otto/plugins/<plugin>/agents/ ab
# $name is the agent file; Hermes runs it in a profile of that name, which
# Hermes has to make.
settings-agent-instructions-hermes = Hermes braucht dafür ein Profil: Führe hermes profile create { $name } aus
# The row that opens the agent file. Its path is shown under it.
settings-agent-instructions-file = Anweisungsdatei
settings-agent-permissions = Berechtigungen
settings-agent-permissions-deny = Immer ablehnen
settings-agent-permissions-ask = Nachfragen
settings-agent-permissions-allow = Immer erlauben
settings-agent-model = Modell
settings-agent-folder = Ordner
# Under an agent's Folder when none is set. $path is Ask's scratch folder,
# such as ~/.local/state/otto/ask.
settings-agent-folder-unset = Nicht festgelegt: Sitzungen starten in { $path }, einem temporären Ordner
settings-agent-folder-detail = Der Agent kann alles darin lesen
# The tint of an agent's cards in Ask. None keeps the plain material.
settings-agent-colour = Farbe
settings-agent-colour-none = Keine
settings-agent-colour-red = Rot
settings-agent-colour-orange = Orange
settings-agent-colour-amber = Bernstein
settings-agent-colour-yellow = Gelb
settings-agent-colour-lime = Limette
settings-agent-colour-green = Grün
settings-agent-colour-teal = Blaugrün
settings-agent-colour-cyan = Cyan
settings-agent-colour-blue = Blau
settings-agent-colour-indigo = Indigo
settings-agent-colour-violet = Violett
...
files-open-with-window-title = Öffnen mit
files-open-with-count =
    { $count ->
        [one] { $count } Datei
       *[other] { $count } Dateien
    }
files-open-with-mixed = Verschiedene Dateitypen
files-open-with-search = Apps durchsuchen
files-open-with-default = Standard
files-open-with-other-apps = Andere Apps
files-open-with-no-match = Keine passenden Apps
...
files-task-copying = { $done } von { $total } werden kopiert
files-task-moving = { $done } von { $total } werden verschoben
# The status bar's fuller line: $name is the file being handled right now.
files-task-progress = { $name } — { $done } von { $total }
files-task-already-running = Eine Aktion nach der anderen — diese läuft noch
...
files-info-text-reading = Wird gelesen…
files-info-text-words =
    { $count ->
        [one] { $count } Wort
       *[other] { $count } Wörter
    }
files-info-text-none = Kein Text
files-info-text-unread = Noch nicht gelesen
...
files-command-recognise-text = Texterkennung ausführen
files-recognise-no-pictures = Hier gibt es keinen Text zu lesen
...
bar-battery-percent = Akku { $percent } %
bar-battery-remaining = Akku { $percent } % — noch { $time }
bar-battery-charging-time = Akku { $percent } % — voll in { $time }
bar-battery-full = Akku { $percent } % — vollständig geladen
bar-battery-charging = Akku { $percent } % — wird geladen
bar-battery-plugged = Akku { $percent } % — angeschlossen, wird nicht geladen
bar-cpu-frequency = CPU { $avg } GHz im Mittel, { $max } GHz Spitze
bar-cpu-governor = Governor: { $governor }
bar-power-saver = Energiesparen
...
schema-frosting-label = Milchglas
...
settings-choice-system-language = Systemsprache
...
launcher-search-ask-agent = Frag @{ $agent }…
...
launcher-ask-loading = Unterhaltung wird geladen…
# The session is going to its terminal window.
launcher-ask-handing-over = Weiter im Terminal…
launcher-ask-handing-over-busy = Erst wird der laufende Vorgang beendet, dann weiter im Terminal…
...
launcher-ask-mode-hint = (Shift+Tab zum Wechseln)
...
launcher-input-yes = Ja
launcher-input-no = Nein
launcher-input-continue = Weiter
launcher-input-skip = Diese überspringen
launcher-input-decline = Nicht antworten
launcher-input-open-link = Link öffnen
# Sends a request that only asked to open a link.
launcher-input-done = Fertig
launcher-input-send = Antworten senden
# Beside an option the agent suggests.
launcher-input-suggested = Vorgeschlagen
# What the empty field says while it takes the answer.
launcher-input-type-answer = Deine Antwort…
launcher-input-type-number = Eine Zahl…
launcher-input-type-other = Oder eine eigene Antwort…
# In the log, above a question when the agent asked several.
launcher-input-progress = Frage { $current } von { $total }
# In the log, a question already answered, and its answer.
launcher-input-answer = { $question }: { $answer }
launcher-input-skipped = Übersprungen
# In the log, under a request nobody answered the usual way.
launcher-input-declined = Abgelehnt
launcher-input-dismissed = Verworfen
launcher-input-unanswered = Nicht beantwortet
# In the log, under a question, when the answer typed can’t be taken.
launcher-input-error-empty = Hier fehlt eine Antwort
launcher-input-error-number = Das muss eine Zahl sein
launcher-input-error-integer = Das muss eine ganze Zahl sein
launcher-input-error-min = Mindestens { $min }
launcher-input-error-max = Höchstens { $max }
launcher-input-error-short = Mindestens { $min } Zeichen
launcher-input-error-long = Höchstens { $max } Zeichen
launcher-input-error-pick-min = Auswahl: mindestens { $min }
launcher-input-error-pick-max = Auswahl: höchstens { $max }
...
agents-permission-title = { $agent } möchte { $action }
# What the tool does, by kind. Each completes "{ $agent } wants to …".
agents-permission-read = eine Datei lesen
agents-permission-edit = eine Datei bearbeiten
agents-permission-delete = eine Datei löschen
agents-permission-move = eine Datei verschieben
agents-permission-search = suchen
agents-permission-execute = einen Befehl ausführen
agents-permission-fetch = etwas aus dem Web laden
agents-permission-switch-mode = die Arbeitsweise ändern
agents-permission-tool = ein Werkzeug verwenden
...
agents-permission-allow = Erlauben
agents-permission-allow-always = Immer erlauben
agents-permission-reject = Ablehnen
...
emoji-no-results = Keine Emojis gefunden
...
islands-music-elsewhere = Wird auf einem anderen Gerät abgespielt
...
islands-dialog-page = { $current } von { $total }
...
islands-dialog-multi-hint = Zutreffendes auswählen
