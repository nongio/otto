    ...
    pub fn init(
        ...
        handle
            .insert_source(remove_workspace_receiver, |event, _, otto| {
                if let ChannelEvent::Msg((index, output_name)) = event {
                    if let Some(ref name) = output_name {
                        // Per-output removal: find position in that output's workspace_views
                        let pos = otto.workspaces.output_workspaces.get(name).and_then(|ows| {
                            ows.workspace_views.iter().position(|w| w.index == index)
                        });
                        if let Some(pos) = pos {
                            otto.workspaces.remove_workspace_at_for_output(pos, name);
                        }
                    } else {
                        // Global removal: find position in the shared model
                        let pos = otto
                            .workspaces
