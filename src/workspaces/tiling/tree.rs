...
impl<L: Clone + Eq + Hash + Debug> Tree<L> {
    ...
    /// Every window leaf's *node id*, in layout order. This is what takes up
    /// space, so it is what the layout counts.
    ...
    /// The window in `id`, when it is a leaf rather than a container.
    pub fn leaf(&self, id: NodeId) -> Option<&L> {
        match self.node(id)? {
            Node::Leaf(l) => Some(l),
            Node::Container { .. } => None,
        }
    ...
    /// The number of cells the layout has to place.
    ...
    /// A container's children, in order. Empty for a leaf.
