...
pub struct PointerMoveSurfaceGrab<B: Backend + 'static> {
    ...
    pub window: WindowElement,
    pub initial_window_location: Point<i32, Logical>,
    /// macOS-style tiling: the zone currently previewed under the cursor while
    /// Super is held during the drag. `None` when no zone is active. Applied on
    /// button release.
...
impl<B: Backend> PointerGrab<Otto<B>> for PointerMoveSurfaceGrab<B> {
    fn motion(
        ...
        // macOS-style tiling: while Ctrl is held, preview the snap zone the
        // pointer is over; release applies it (see `button`).
    ...
    fn button(
        ...
        if handle.current_pressed().is_empty() {
            ...
            // macOS-style tiling: snap the window into the previewed zone, if any.
