...
use crate::{app_runner::{App, AppContext}, components::menu::sc_layer_v1::BlendMode};
use crate::components::menu::sc_layer_v1;
use crate::surfaces::{SubsurfaceSurface, Surface, SurfaceError, ToplevelSurface};
...
impl ApplicationWindow {
    ...
    pub fn new<A: App + 'static>(
        ...
        // Get required states from AppContext
    ...
    fn create_titlebar<A: App + 'static>(&self) -> Result<(), SurfaceError> {
        ...
        let wl_surface = self.wl_surface()
            .ok_or_else(|| SurfaceError::WaylandError("Toplevel surface not available".to_string()))?;
