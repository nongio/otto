...
use hello_design::{
    components::container::{
        BoxShadow, Container, FrameBuilder, Stack,
        StackDirection,
    },
...
impl ContainerDemo {
    ...
    fn render_simple_frames(canvas: &Canvas, x: f32, y: f32) {
        ...
        canvas.draw_str("Three different colored frames", (x, y - 5.0), &font, &paint);
    }
...
impl App for ContainerDemo {
    fn on_app_ready(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        let mut window = Window::new::<ContainerDemo>("Container Demo", 600, 550)?;
